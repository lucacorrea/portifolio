        ...
        <?php foreach ($budgets as $budget): ?>
            ...
            <tr>
                ...
                <td class="table-actions-cell"><div class="dropdown table-action-dropdown"><button class="btn-action" type="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Ações do orçamento <?= h($budget->displayNumber()) ?>"><i class="bi bi-three-dots-vertical"></i></button><ul class="dropdown-menu dropdown-menu-end">
                    ...
                    <?php if ($canDelete && $budget->status() !== 'aprovado'): ?><li><hr class="dropdown-divider"></li><li><button class="dropdown-item text-danger js-budget-delete" type="button" data-budget-id="<?= h((string) $budget->id()) ?>" data-budget-number="<?= h($budget->displayNumber()) ?>" data-bs-toggle="modal" data-bs-target="#modal-orcamento-delete"><i class="bi bi-trash3"></i> Excluir orçamento</button></li><?php endif; ?>
                </ul></div></td>
            </tr>
        <?php endforeach; ?>
...
<?php if ($canDelete): ?><div class="modal fade" id="modal-orcamento-delete" tabindex="-1" aria-labelledby="budget-delete-title" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><form class="modal-content visual-modal" method="post" action="actions/orcamento-excluir.php"><div class="modal-header"><h2 class="modal-title fs-5" id="budget-delete-title">Excluir orçamento</h2><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button></div><div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="delete-budget-id"><p>Deseja excluir <strong id="delete-budget-number"></strong>?</p><div class="alert alert-warning mb-0">Os itens e o histórico serão preservados. Orçamento aprovado ou com OS vinculada não pode ser excluído.</div></div><div class="modal-footer"><button class="btn-modal-cancel" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn-modal-save" type="submit"><i class="bi bi-trash3"></i> Excluir orçamento</button></div></form></div></div><?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', async function (event) {
        ...
        if (button.classList.contains('js-budget-delete')) { document.getElementById('delete-budget-id').value = button.dataset.budgetId || ''; document.getElementById('delete-budget-number').textContent = button.dataset.budgetNumber || 'este orçamento'; }
    });
