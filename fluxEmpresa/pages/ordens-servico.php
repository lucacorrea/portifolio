                        ...
                        <td class="table-actions-cell">
                            <div class="dropdown table-action-dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    ...
                                    <?php if ($canFinalize && $isSoImported && $order->status() === 'aguardando_agendamento'): ?><li><button class="dropdown-item js-os-finalize-import" type="button" data-order-id="<?= h((string) $order->id()) ?>" data-order-number="<?= h($order->displayNumber()) ?>" data-bs-toggle="modal" data-bs-target="#modal-os-finalize-import"><i class="bi bi-check2-circle"></i> Revisar e finalizar importação</button></li><?php endif; ?>
                                    ...
                                    <?php if ($canFinalize && in_array($order->status(), ['agendada','em_execucao','aguardando_peca'], true)): ?><li><button class="dropdown-item js-os-finalize" type="button" data-order-id="<?= h((string) $order->id()) ?>" data-order-number="<?= h($order->displayNumber()) ?>" data-order-total="<?= h($order->total()) ?>" data-order-items="<?= h((string) $order->itemsCount()) ?>" data-bs-toggle="modal" data-bs-target="#modal-os-finalize"><i class="bi bi-check2-circle"></i> Finalizar OS</button></li><?php endif; ?>
...
<div class="modal fade" id="modal-os-finalize" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-lg modal-dialog-centered"><form class="modal-content visual-modal" method="post" action="actions/os-finalizar.php"><div class="modal-header"><div><h2 class="modal-title fs-5">Finalizar OS</h2><p class="text-muted small mb-0">Confirme a conclusão usando os itens e o valor já registrados na OS.</p></div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button></div><div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="os-finalize-id"><div class="alert alert-info mb-3" role="note"><i class="bi bi-info-circle"></i> Nenhum serviço ou valor precisa ser digitado novamente.</div><section class="form-section mb-0"><h3 class="form-section-title">Resumo da finalização</h3><div class="summary-box"><div><span>OS</span><strong id="os-finalize-number">-</strong></div><div><span>Valor aprovado</span><strong id="os-finalize-total">Carregando...</strong></div><div><span>Itens</span><strong id="os-finalize-items">-</strong></div></div><p class="text-muted small mb-0 mt-3">Após concluir, perguntaremos se o cliente já pagou. Se não, a cobrança ficará pendente em Contas a Receber.</p></section></div><div class="modal-footer"><button class="btn-modal-cancel" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn-modal-save" id="os-finalize-submit" type="submit"><i class="bi bi-check2-circle"></i> Confirmar finalização</button></div></form></div></div>
<div class="modal fade" id="modal-os-finalize-import" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-xl modal-dialog-scrollable"><form class="modal-content visual-modal" method="post" action="actions/os-finalizar-importacao-so.php"><div class="modal-header"><div><h2 class="modal-title fs-5">Revisar importação do SO</h2><p class="text-muted small mb-0" id="os-finalize-import-subtitle"></p></div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button></div><div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="os-finalize-import-id"><div class="alert alert-info" role="note"><i class="bi bi-info-circle"></i> Serviços serão finalizados como execução. Peças e insumos externos não movimentam estoque até serem cadastrados como produtos.</div><div class="alert alert-danger" id="os-finalize-import-error" role="alert" hidden></div><div class="table-panel-wrap"><table class="os-table"><thead><tr><th>Item importado</th><th>Qtd.</th><th>Valor</th><th>Classificação</th></tr></thead><tbody id="os-finalize-import-items"></tbody></table></div></div><div class="modal-footer"><button class="btn-modal-cancel" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn-modal-save" id="os-finalize-import-submit" type="submit"><i class="bi bi-check2-circle"></i> Finalizar OS</button></div></form></div></div>
