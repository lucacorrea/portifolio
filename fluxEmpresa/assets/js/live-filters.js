document.addEventListener('DOMContentLoaded', () => {
  ...
  if (actionDialog && actionHost && typeof actionDialog.showModal === 'function') {
    window.FluxEmpresas = window.FluxEmpresas || {};
    window.FluxEmpresas.refreshRecordActions = enhanceRecordActions;
  ...
  const enhanceForm = (form) => {
    ...
    const run = async ({ url = requestUrl(form), updateHistory = true } = {}) => {
      ...
      try {
        ...
        window.FluxEmpresas?.refreshActionTables?.();
        window.FluxEmpresas?.refreshRecordActions?.();
        ...
        errorBox.classList.add('d-none');
        document.dispatchEvent(new CustomEvent('fluxempresas:live-filter-updated', {
