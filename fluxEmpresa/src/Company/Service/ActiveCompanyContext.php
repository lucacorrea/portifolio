...
final class ActiveCompanyContext
{
    ...
    public function current(): ?ActiveCompany
    {
        ...
        if (!is_array($data)
            || !isset($data['id'], $data['uuid'], $data['name'], $data['support_user_id'], $data['access_id'], $data['entered_at'])
            || (int) $data['id'] <= 0
            || (int) $data['support_user_id'] <= 0
            || (int) $data['access_id'] <= 0
            || trim((string) $data['name']) === ''
            || strlen((string) $data['name']) > 180
            || strlen((string) $data['uuid']) > 36
            || strtotime((string) $data['entered_at']) === false
        ) {
            $this->clear();
            return null;
        }
        ...
        return new ActiveCompany(
            ...
            (string) $data['uuid'],
            (string) $data['name'],
            (int) $data['support_user_id'],
            (int) $data['access_id'],
            (string) $data['entered_at']
        );
    }
