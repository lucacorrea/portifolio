...
financialFlowAssert(str_contains((string) $finalizationAction, 'getOrderItems($orderId)'), 'Finalização deve usar os itens já persistidos na OS.');
financialFlowAssert(str_contains((string) $finalizationAction, "'desconto' => \$order->discount()"), 'Finalização deve preservar o desconto do orçamento/OS.');
financialFlowAssert(str_contains((string) $finalizationAction, "'acrescimo' => \$order->increase()"), 'Finalização deve preservar o acréscimo do orçamento/OS.');
