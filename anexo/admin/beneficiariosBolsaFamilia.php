...
/* ===== Dados (client-side paging) ===== */
try {
    $sql = "SELECT id, nome, cpf, pbf_valor, numero, endereco, telefone, responsavel
            FROM solicitantes
            WHERE pbf = 'Sim'
    ...
    http_response_code(500);
    echo "<div class='alert alert-danger'>Erro ao consultar beneficiários: " . h($e->getMessage()) . "</div>";
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="utf-8">
    <title>Beneficiários – Bolsa Família</title>
            ...
            <div class="sidebar-wrapper active">
                <div class="sidebar-header">
                    <div class="d-flex justify-content-between">
                        <div class="logo"><a href="dashboard.php"><img src="../dist/assets/images/logo/logo_pmc_2025.jpg" alt="Logo" style="height:48px"></a></div>
                    ...
                    <ul class="menu">
                        ...
                        <li class="sidebar-item has-sub active">
                            ...
                            <ul class="submenu active">
                                <li class="submenu-item active"><a href="#">Bolsa Família</a></li>
        ...
        <div id="main" class="d-flex flex-column min-vh-100">
            ...
            <div class="page-heading">
                <div class="page-title">
                    <div class="row">
                        <div class="col-12 col-md-6">
                            <h3>Beneficiários – Bolsa Família</h3>
                            <p class="text-subtitle text-muted">Lista de beneficiários do programa federal</p>
                        </div>
                        <div class="col-12 col-md-6">
                            <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="#">Beneficiarios</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">Bolsa Família</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                ...
                <section class="section mb-4">
                    <div class="card">
                        <!-- Cabeçalho com ESPAÇO entre input e X -->
                        <div class="card-header card-header-grid">
                            <span class="fw-semibold">Lista de Beneficiários</span>
                            <div class="search-inline">
                                <input type="text" id="inpSearch" class="form-control form-control-sm"
                                    placeholder="Buscar por nome/CPF/telefone/endereço/responsável..." autocomplete="off">
                                <button class="btn btn-sm btn-outline-dark" id="btnClear" title="Limpar">
                                        ...
                                        <?php if (!$rows): ?>
                                            ...
                                            <?php else: foreach ($rows as $r): ?>
                                                ...
                                                <tr
                                                    ...
                                                    <td><?= h($r['nome']) ?></td>
                                                    <td class="text-end"><?= h(fmt_money($r['pbf_valor'])) ?></td>
                                                    <td><?= h($r['numero']) ?></td>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="profile-wrap">
                        <img id="md-foto" class="modal-photo" src="assets/images/placeholders/user-placeholder.svg" alt="Foto">
    ...
    <script>
        (() => {
            ...
            document.addEventListener('click', async (ev) => {
                ...
                const d = j.solicitante || {};
                const documentos = Array.isArray(j.documentos) ? j.documentos : [];
                const familiares = Array.isArray(j.familiares) ? j.familiares : [];
                ...
                setText('md-criado', d.created_at ? fmtDateTime(d.created_at) : '—');
                ...
                const anos = Number(d.tempo_anos || 0),
                    meses = Number(d.tempo_meses || 0);
                setText('md-tempo', (anos || meses) ? (anos + ' ano(s)' + (meses ? (', ' + meses + ' mês(es)') : '')) : '—');
                ...
                setText('md-bairro', d.bairro_nome || '—');
                ...
                if (documentos.length) {
                    documentos.forEach(doc => {
                        ...
                        const when = doc.created_at ? fmtDateTime(doc.created_at) : '';
                        ...
                        // 👉 monta o caminho corretamente com ../dist/
                ...
                const famBody = document.getElementById('md-familiares');
                famBody.innerHTML = '';
                if (familiares.length) {
                    familiares.forEach(f => {
                        const tr = document.createElement('tr');
                        tr.innerHTML = `<td>${f.nome||'—'}</td><td>${fmtDate(f.data_nascimento)}</td>
                              <td>${f.parentesco||'—'}</td><td>${f.escolaridade||'—'}</td><td>${f.obs||''}</td>`;
                        famBody.appendChild(tr);
                    });
                } else {
                    famBody.innerHTML = `<tr><td colspan="5" class="text-center text-muted">Sem familiares cadastrados.</td></tr>`;
                }
