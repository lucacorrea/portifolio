...
try {
    $stmt = $pdo->prepare("SELECT id, nome, autorizado FROM contas_acesso_privado WHERE id=:id LIMIT 1");
    $stmt->execute([':id' => $id]);
    ...
    $up = $pdo->prepare("UPDATE contas_acesso_privado SET autorizado='nao', updated_at=NOW() WHERE id=:id LIMIT 1");
    $up->execute([':id' => $id]);
