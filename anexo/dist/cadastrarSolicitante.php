...
<head>
    ...
    <title>Novo Solicitante (Multi-etapas) - ANEXO</title>
...
<body>
                            ...
                            <section class="form-section" data-form-step="1">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <div class="text-danger small mt-1 d-none" id="cpf-exists">
                                    ...
                                    <script>
                                        ...
                                        const cpfExists = document.getElementById('cpf-exists');
                                        ...
                                        function verificarCPF() {
                                            ...
                                            cpfExists.classList.add('d-none');
                                            cpfInvalid.textContent = 'CPF inválido (11 dígitos).';
                                            ...
                                            formData.append('acao', 'verificar_cpf');
                                            ...
                                            fetch('', {
                                                ...
                                                .then(resp => {
                                                    if (resp.exists) {
                                                        cpfInput.classList.add('is-invalid');
                                                        cpfExists.classList.remove('d-none');
                                                        cpfInvalid.textContent = '';
                                                    } else {
                                        ...
                                        cpfInput.addEventListener('input', () => {
                                            ...
                                            debounceTimer = setTimeout(verificarCPF, 400); // 0.4s
                                        });
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select" name="trabalho" id="trabalho">
                                            <option value="">Selecione…</option>
                                            <?= opt('Empregado(a)') . opt('Desempregado(a)') . opt('Autônomo(a)') . opt('Aposentado(a)/Pensionista') . opt('Outros') ?>
                                        </select>
                                    </div>
                                    <div class="col-md-6 d-none" id="wrap_local_trabalho">
                                        <label class="form-label">Local de Trabalho</label>
                                        <input type="text" class="form-control" name="local_trabalho" id="local_trabalho" placeholder="Informe o local de trabalho">
                                    </div>
                                    <div class="col-md-6">
    ...
    <script>
        ...
        (function() {
            ...
            const trabalhoSel = document.getElementById('trabalho');
            const wrapLocalTrabalho = document.getElementById('wrap_local_trabalho');
            ...
            function toggleLocalTrabalho() {
                const isEmpregado = (trabalhoSel?.value || '') === 'Empregado(a)';
                toggle(wrapLocalTrabalho, isEmpregado);
                if (!isEmpregado) document.getElementById('local_trabalho').value = '';
            }
            ...
            trabalhoSel?.addEventListener('change', toggleLocalTrabalho);
            ...
            function buildResumo() {
                ...
                if (document.getElementById('foto_base64').value) {
                    html += kv('Foto do Solicitante', 'Capturada na câmera (Base64)');
                } else if (fotoFile) {
                    html += kv('Foto do Solicitante', `Arquivo: ${fotoFile.name} (${(fotoFile.size / 1024 / 1024).toFixed(2)} MB)`);
                } else {
                    html += kv('Foto do Solicitante', 'Não informada');
                }
                html += '</div>';
      ...
      <td class="text-center">
        <button class="btn btn-outline-danger btn-icon-sm" type="button" title="Remover"><i class="bi bi-trash"></i></button>
            ...
            // Ticker Relógio (Hora do Cadastro)
            ...
            function tickHora() {
                if (!elHora) return;
                const d = new Date();
                elHora.value = `${pad2(d.getHours())}:${pad2(d.getMinutes())}:${pad2(d.getSeconds())}`;
        ...
        (function() {
            ...
            function startCamera() {
                ...
                navigator.mediaDevices.getUserMedia(constraints)
                    ...
                    .catch(e => {
                        console.error('Erro na primeira tentativa de câmera:', e);
                        // Tenta fallback sem constraint (Chrome Desktop etc)
                        navigator.mediaDevices.getUserMedia({
                                video: true
                            })
                            .then(s => {
                                console.log('Stream obtido via fallback:', s);
                                stream = s;
                                if (video) {
                                    video.srcObject = s;
                                    video.onloadedmetadata = () => {
                                        video.play().catch(e => console.error('Erro ao dar play no vídeo (fallback):', e));
                                    };
                                }
                            })
                            .catch(err => {
                                console.error('Erro total ao acessar câmera:', err);
                                alert('Não foi possível acessar a câmera do perfil. Certifique-se de estar usando HTTPS e que deu permissão.');
                            });
                    });
            }
            ...
            btnCapture?.addEventListener('click', () => {
                if (!stream) {
                    alert('Aguarde a câmera carregar ou recarregue a página.');
                    return;
                }
                ...
                // Otimização: Limita largura para evitar payloads gigantes
                const maxW = 1280;
                ...
                // Qualidade 0.7 para reduzir tamanho do arquivo
            ...
            btnUse?.addEventListener('click', () => {
                try {
                    if (!imgPreview.src || imgPreview.src.length < 100) {
                        alert("Por favor, capture uma foto primeiro.");
                        return;
                    }

                    // Converte Base64 para ARQUIVO real para evitar erro 403 (ModSecurity)
                    const file = dataURLtoFile(imgPreview.src, "foto_perfil.jpg");
                    ...
                    // Limpa o campo oculto base64 pois a foto agora vai via multipart/form-data
                    inputBase64.value = "";

                    // Exibe confirmação no formulário
                    const statusEl = document.getElementById('fotoStatusText');
                    if (statusEl) {
                        statusEl.textContent = "✅ Foto capturada da câmera";
                    ...
                    alert('Erro ao processar a foto da câmera: ' + err.message);
                }
            });
        })();
        ...
        (function() {
            ...
            function startScanCamera() {
                console.log('Iniciando câmera do scanner...');
                ...
                navigator.mediaDevices.getUserMedia(constraints)
                    .then(s => {
                        console.log('Stream obtido (scan):', s);
                        stream = s;
                        if (video) {
                            video.srcObject = s;
                            video.onloadedmetadata = () => {
                                video.play().catch(e => console.error('Erro play scanner:', e));
                            };
                        }
                    })
                    .catch(e => {
                        console.error('Erro scanner fallback (scan):', e);
                        // Fallback
                        navigator.mediaDevices.getUserMedia({
                                video: true
                            })
                            .then(s => {
                                console.log('Stream obtido via fallback (scan):', s);
                                stream = s;
                                if (video) {
                                    video.srcObject = s;
                                    video.onloadedmetadata = () => {
                                        video.play().catch(e => console.error('Erro play scanner (fallback):', e));
                                    };
                                }
                            })
                            .catch(err => {
                                console.error('Erro total scanner:', err);
                                alert('Erro ao acessar câmera do scanner: ' + err.message);
                            });
                    });
            }
