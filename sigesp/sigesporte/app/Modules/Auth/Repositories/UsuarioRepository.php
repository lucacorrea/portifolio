...
{
    public function findByIdentifier(string $identifier): ?array { $q = Database::connection()->prepare('SELECT * FROM usuarios WHERE (email = :identifier OR cpf = :cpf) AND ativo = 1 AND deleted_at IS NULL LIMIT 1'); $q->execute(['identifier' => mb_strtolower($identifier), 'cpf' => preg_replace('/\D/', '', $identifier)]); return $q->fetch() ?: null; }
