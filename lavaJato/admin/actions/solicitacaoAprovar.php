 ...
 * Evita erro fatal caso require_post() não exista no auth_guard.php
 */
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: ../pages/solicitacao.php?err=1&msg=' . urlencode('Método inválido.'));
    exit;
}
...
// CSRF
$csrf = $_POST['csrf'] ?? '';
if (empty($_SESSION['csrf_admin']) || !hash_equals((string)$_SESSION['csrf_admin'], (string)$csrf)) {
    header('Location: ../pages/solicitacao.php?err=1&msg=' . urlencode('Token inválido.'));
    exit;
}
...
if ($solId <= 0) {
    header('Location: ../pages/solicitacao.php?err=1&msg=' . urlencode('Solicitação inválida.'));
    exit;
}

if (!isset($pdo) || !($pdo instanceof PDO)) {
    header('Location: ../pages/solicitacao.php?err=1&msg=' . urlencode('Conexão com banco indisponível.'));
    exit;
}
...
/** Monta URL absoluta baseada no host atual + BASE (se definida) */
...
try {
    ...
    if (!$sol) {
        header('Location: ../pages/solicitacao.php?err=1&msg=' . urlencode('Solicitação não encontrada.'));
        exit;
    }

    if (($sol['status'] ?? '') !== 'pendente') {
        header('Location: ../pages/solicitacao.php?err=1&msg=' . urlencode('Solicitação já processada.'));
        exit;
    }

    // Se não veio CNPJ no POST, usa o da solicitação
    if ($cnpj === '') {
        $cnpj = preg_replace('/\D+/', '', (string)($sol['cnpj'] ?? ''));
    }

    if (strlen($cnpj) !== 14) {
        header('Location: ../pages/solicitacao.php?err=1&msg=' . urlencode('CNPJ obrigatório para aprovar.'));
        exit;
    }
    ...
    if (!$empresa) {
        $ins = $pdo->prepare("
            INSERT INTO empresas_peca
                (cnpj, nome_fantasia, email, telefone, status, criado_em)
            VALUES
                (:cnpj, :nome, :email, :tel, 'ativa', NOW())
        ");
        $ins->execute([
            ':cnpj'  => $cnpj,
            ':nome'  => (string)($sol['nome_fantasia'] ?? ''),
    ...
    $upSol = $pdo->prepare("
        UPDATE solicitacoes_empresas_peca
           SET status = 'aprovada',
               cnpj   = :cnpj
         WHERE id = :id
    ");
    ...
    if ($senhaParaGravar === '') {
        $senhaTemporaria = bin2hex(random_bytes(6));
    ...
    header('Location: ../pages/solicitacao.php?ok=1&msg=' . urlencode('Solicitação aprovada. Dono criado/atualizado e e-mail enviado.'));
    exit;
    ...
    header('Location: ../pages/solicitacao.php?err=1&msg=' . urlencode('Falha ao aprovar: ' . $e->getMessage()));
    exit;
}