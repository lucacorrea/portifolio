...
/* ===========================================================
   MINI API: ?solicitacoes=1&id=123 -> solicitações do solicitante
   =========================================================== */
if (($_GET['solicitacoes'] ?? '') === '1') {
  header('Content-Type: application/json; charset=UTF-8');
  $id = (int)($_GET['id'] ?? 0);
  $cpfParam = only_digits((string)($_GET['cpf'] ?? ''));

  if ($id <= 0 && strlen($cpfParam) !== 11) {
    echo json_encode(['ok' => false, 'error' => 'Informe um ID ou CPF válido'], JSON_UNESCAPED_UNICODE);
    exit;
  }
  ...
  try {
    ...
    if ($id > 0) {
      $whereSolicitante[] = 'id = :id';
      $paramsSolicitante[':id'] = $id;
    }

    if (strlen($cpfParam) === 11) {
      $whereSolicitante[] = 'cpf = :cpf_lookup';
      $paramsSolicitante[':cpf_lookup'] = $cpfParam;
    }

    $orderSolicitante = 'ORDER BY id DESC';
    if ($id > 0) {
      $orderSolicitante = 'ORDER BY CASE WHEN id = :id_order THEN 0 ELSE 1 END, id DESC';
      $paramsSolicitante[':id_order'] = $id;
    }

    $st = $pdo->prepare("SELECT id, nome, cpf
                         FROM solicitantes
                         WHERE " . implode(' OR ', $whereSolicitante) . "
                         {$orderSolicitante}
                         LIMIT 1");
    $st->execute($paramsSolicitante);
    $sol = $st->fetch(PDO::FETCH_ASSOC);
    if (!$sol) {
      echo json_encode(['ok' => false, 'error' => 'Solicitante não encontrado'], JSON_UNESCAPED_UNICODE);
      exit;
    }
    ...
    $req = $pdo->prepare("
      SELECT
        ...
        so.solicitante_id,
        so.ajuda_tipo_id,
        COALESCE(at.nome, 'Não informado') AS ajuda_nome,
        so.resumo_caso,
        so.data_solicitacao,
        so.status,
        so.created_by
      FROM solicitacoes so
      LEFT JOIN solicitantes sr ON sr.id = so.solicitante_id
      LEFT JOIN ajudas_tipos at ON at.id = so.ajuda_tipo_id
      WHERE so.solicitante_id = :sid
         OR sr.cpf = :cpf
      ORDER BY so.data_solicitacao DESC, so.id DESC
      LIMIT 500
    ");
    $req->execute([
      ':sid' => (int)$sol['id'],
    ...
    $solicitacoes = $req->fetchAll(PDO::FETCH_ASSOC) ?: [];
    ...
    echo json_encode(['ok' => false, 'error' => 'Falha ao consultar solicitações'], JSON_UNESCAPED_UNICODE);
    exit;
  }
}
...
<body>
                        ...
                        <tr
                          ...
                          <td class="text-center table-actions">
                            <div class="btn-group btn-group-sm" role="group">
                              ...
                              <button type="button"
                                class="btn btn-outline-success btnSolicitacoes"
                                title="Ver solicitações">
                                <i class="bi bi-list-check"></i> Solicitações
                              </button>
                            </div>
                          </td>
                        </tr>
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <a id="btnModalEditar" class="btn btn-primary disabled" href="#" aria-disabled="true">
  ...
  <div class="modal fade" id="modalSolicitacoes" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <div>
            <h5 class="modal-title">Solicitações do Beneficiário</h5>
            <div class="text-muted small" id="ms-subtitle">—</div>
          </div>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
        </div>

        <div class="modal-body">
          <div class="solicitacoes-summary">
            <span class="badge-lite"><i class="bi bi-person"></i> <span id="ms-nome">—</span></span>
            <span class="badge-lite"><i class="bi bi-card-list"></i> <span id="ms-cpf">—</span></span>
            <span class="badge-lite"><i class="bi bi-list-check"></i> <span id="ms-total">0 solicitação(ões)</span></span>
          </div>
          <div id="ms-lista" class="solicitacoes-list">
            <div class="text-muted">Carregando solicitações...</div>
          </div>
        </div>

        <div class="modal-footer">
          <a id="btnModalSolicitacoesEditar" class="btn btn-primary disabled" href="#" aria-disabled="true">
            <i class="bi bi-pencil"></i> Editar
          </a>
          <button class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
        </div>
  ...
  <script>
    (() => {
      ...
      const setEditLink = (id, cpf) => {
        const a = document.getElementById(id);
        if (!a) return;

        const cpfDigits = String(cpf || '').replace(/\D+/g, '');
        if (cpfDigits.length === 11) {
          a.href = `editar.php?cpf=${encodeURIComponent(cpfDigits)}`;
      ...
      const statusClass = (status) => {
        ...
        if (['concluido', 'concluído', 'fechado', 'atendido', 'aprovado'].includes(s)) return 'badge bg-success';
        if (['cancelado', 'negado', 'recusado', 'indeferido'].includes(s)) return 'badge bg-danger';
        if (['andamento', 'em andamento', 'pendente', 'analise', 'análise'].includes(s)) return 'badge bg-warning text-dark';
        return 'badge bg-primary';
      };

      document.addEventListener('click', async (ev) => {
        const btn = ev.target.closest?.('.btnSolicitacoes');
        if (!btn) return;
        ...
        setText('ms-subtitle', 'Carregando histórico...');
        setText('ms-nome', '—');
        setText('ms-cpf', '—');
        setText('ms-total', '0 solicitação(ões)');
        setEditLink('btnModalSolicitacoesEditar', cpf);
        box.innerHTML = '<div class="text-muted">Carregando solicitações...</div>';

        const modal = new bootstrap.Modal(modalEl);
        modal.show();

        const url = new URL(location.pathname, location.origin);
        url.searchParams.set('solicitacoes', '1');
        ...
        if (!j.ok) {
          setText('ms-subtitle', 'Não foi possível carregar as solicitações.');
          box.innerHTML = '';
          const err = mk('div', 'alert alert-danger mb-0');
          err.textContent = j.error || 'Não foi possível carregar as solicitações.';
          box.appendChild(err);
          return;
        }

        const s = j.solicitante || {};
        const solicitacoes = Array.isArray(j.solicitacoes) ? j.solicitacoes : [];

        setText('ms-subtitle', 'Histórico de solicitações registradas');
        setText('ms-nome', s.nome || '—');
        setText('ms-cpf', maskCPF(s.cpf || ''));
        setText('ms-total', `${solicitacoes.length} solicitação(ões)`);
        setEditLink('btnModalSolicitacoesEditar', s.cpf || cpf);

        box.innerHTML = '';
        if (!solicitacoes.length) {
          box.innerHTML = '<div class="text-muted">Nenhuma solicitação registrada para este beneficiário.</div>';
          return;
        }

        solicitacoes.forEach(req => {
          const item = mk('div', 'solicitacao-item');

          const head = mk('div', 'solicitacao-head');
          const title = mk('div', 'solicitacao-title');
          title.textContent = req.ajuda_nome || 'Solicitação';

          const status = mk('span', statusClass(req.status));
          status.textContent = req.status || 'Aberto';

          head.appendChild(title);
          head.appendChild(status);
          item.appendChild(head);

          const meta = mk('div', 'solicitacao-meta');
          meta.appendChild(badge('<i class="bi bi-calendar-event"></i>', fmtDateTimeBR(req.data_solicitacao || '')));
          meta.appendChild(badge('<i class="bi bi-person-check"></i>', req.created_by || 'Não informado'));
          meta.appendChild(badge('<i class="bi bi-hash"></i>', `Solicitação ${req.id || '—'}`));
          item.appendChild(meta);

          const resumo = mk('div', 'solicitacao-resumo');
          const resumoTxt = String(req.resumo_caso || '').trim();
          resumo.textContent = resumoTxt || 'Sem resumo informado.';
          if (!resumoTxt) resumo.classList.add('text-muted');
      ...
      document.addEventListener('click', async (ev) => {
        ...
        setEditLink('btnModalEditar', s.cpf || tr?.dataset?.cpf || '');

        const placeholder = '../dist/assets/images/placeholder-user.jpg';
