...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_ajax'] ?? '') === '1') {
    ...
    $check = $pdo->prepare("
        SELECT ae.id, ae.foto_path
        FROM ajudas_entregas ae
        INNER JOIN solicitantes s ON s.id = ae.pessoa_id
        WHERE s.cpf = :cpf
        ORDER BY ae.id DESC
        LIMIT 1
    ");
    ...
    try {
        ...
        if ($quantidade < 1) {
            echo json_encode(['ok' => false, 'error' => 'Quantidade inválida.']);
            exit;
        }

        if ($ajudaTipoId <= 0) {
            echo json_encode(['ok' => false, 'error' => 'Selecione o tipo de entrega.']);
            exit;
        }

        $validaTipo = $pdo->prepare("SELECT id FROM ajudas_tipos WHERE id = :id LIMIT 1");
        $validaTipo->execute([':id' => $ajudaTipoId]);
        if (!$validaTipo->fetchColumn()) {
        ...
        if ($fotoBase64 !== '' && str_contains($fotoBase64, ',')) {
            $ext = 'jpg';
            if (str_contains($fotoMime, 'png')) {
                $ext = 'png';
            } elseif (str_contains($fotoMime, 'webp')) {
                $ext = 'webp';
            }
...
try {
    $st = $pdo->prepare("
        SELECT
            ae.*,
            s.nome AS pessoa_nome,
            s.cpf AS pessoa_cpf,
            s.telefone AS pessoa_telefone,
            s.endereco AS pessoa_endereco,
            s.numero AS pessoa_numero,
...
/* ===== Carrega tipos de ajuda para o select ===== */
$tiposAjuda = [];
try {
    $stTipos = $pdo->query("
        SELECT id, nome
        ...
        ORDER BY nome ASC
    ");
    $tiposAjuda = $stTipos->fetchAll(PDO::FETCH_ASSOC) ?: [];
} catch (Throwable $e) {
    $tiposAjuda = [];
}
...
$d = [
    ...
    'ajuda_tipo_id'           => (int)($entrega['ajuda_tipo_id'] ?? 0),
                ...
                <section class="section">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form id="formEditar" class="form-grid" novalidate>
                                <input type="hidden" name="cpf" value="<?= $d['cpf_only_digits'] ?>">
                                ...
                                <div class="field">
                                    ...
                                    <select name="ajuda_tipo_id" id="ajuda_tipo_id" class="form-select" required>
                                        ...
                                        <?php foreach ($tiposAjuda as $tipo): ?>
                                            <option value="<?= (int)$tipo['id'] ?>" <?= $d['ajuda_tipo_id'] === (int)$tipo['id'] ? 'selected' : '' ?>>
                                                <?= e((string)$tipo['nome']) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <div class="help">Selecione o benefício entregue para esta pessoa.</div>
                                </div>
