...
{
  $beneficioNorm = normalizeSearchTerm((string)$beneficio);
  return in_array($beneficioNorm, ['atendimento ao prefeito', 'emprego'], true);
}
...
function buildFilters(PDO $pdo, array $in): array
{
  ...
  addInFilter($where, $params, 's.ajuda_tipo_id', 'tid', $beneficioIds);
...
function fetchSolicitationsForExport(PDO $pdo, array $in, int $maxRows = 10000): array
{
  ...
  addInFilter($where, $params, 'sol.ajuda_tipo_id', 'tid', $beneficioIds);

  // Emprego pretendido filtrado pelo resumo da solicitação
  addEmploymentSummaryFilter($pdo, $where, $params, 'sol.resumo_caso', $empregoIds, 'solemp');
  ...
  $sql = "
    SELECT
      ...
      DATE(sol.data_solicitacao) AS data_solicitacao,
      sol.resumo_caso
    FROM solicitacoes sol
    JOIN solicitantes s ON s.id = sol.solicitante_id
    LEFT JOIN bairros b ON b.id = s.bairro_id
          ...
          <div class="card">
            <div class="card-header">
              ...
              <div class="actions">
                ...
                <button type="button" id="btnExportPDF" class="btn btn-outline-primary">
