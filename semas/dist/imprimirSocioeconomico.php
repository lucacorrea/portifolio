...
function fmt_valor_entrega_folha($n): string
{
  if ($n === null || $n === '') return '';
 ...
 * - Bloqueia ".."
 * - Aceita uploads/fotos/arquivo.jpg ou apenas o nome do arquivo
 * Retorna caminho relativo para o navegador ou '' se inválido.
 */
function photo_src(?string $path): string
{
  ...
  $uploadsPos = strpos($p, 'uploads/');
  if ($uploadsPos !== false) {
    $p = substr($p, $uploadsPos);
  }
...
/**
 * Pega status da entrega. Quando houver solicitacao_id, a checagem é por solicitação.
 * O fallback por CPF + tipo fica apenas para dados antigos sem solicitacao_id.
 */
function status_por_entrega(PDO $pdo, string $cpf, ?int $ajudaTipoId = null, ?int $solicitacaoId = null, ?int $pessoaId = null): array
{
  $cpf = only_digits($cpf);
  $ajudaTipoId = $ajudaTipoId ? (int)$ajudaTipoId : null;
  $solicitacaoId = $solicitacaoId ? (int)$solicitacaoId : null;
  $pessoaId = $pessoaId ? (int)$pessoaId : null;
  ...
  $sql = "SELECT data_entrega, hora_entrega, responsavel, quantidade, valor_aplicado
          ...
          WHERE UPPER(entregue) IN ('SIM','1')";
  $params = [];

  if ($solicitacaoId && table_has_column($pdo, 'ajudas_entregas', 'solicitacao_id')) {
    $sql .= " AND solicitacao_id = :solicitacao_id";
    $params[':solicitacao_id'] = $solicitacaoId;
  } else {
    $wherePessoa = [];
    if ($cpf !== '') {
      $wherePessoa[] = "pessoa_cpf = :cpf";
      $params[':cpf'] = $cpf;
    }
    if ($pessoaId) {
      $wherePessoa[] = "pessoa_id = :pessoa_id";
    ...
    $sql .= " AND (" . implode(' OR ', $wherePessoa) . ")";

    if ($ajudaTipoId) {
      $sql .= " AND ajuda_tipo_id = :tid";
      $params[':tid'] = $ajudaTipoId;
    }
  }
  ...
  try {
    ...
    if ($row) {
      ...
      return [
        ...
        'valor_folha' => fmt_valor_entrega_folha($row['valor_aplicado'] ?? null),
      ];
    }
  } catch (Throwable $e) {
...
if ($idParam <= 0 && strlen($cpfParam) !== 11) {
  http_response_code(400);
  exit('Informe um ID ou CPF válido.');
}
...
$paramsSolicitante = [];
if ($idParam > 0) {
  $whereSolicitante[] = 's.id = :id';
  $paramsSolicitante[':id'] = $idParam;
}
if (strlen($cpfParam) === 11) {
  $whereSolicitante[] = 's.cpf = :cpf';
  $paramsSolicitante[':cpf'] = $cpfParam;
}
$orderSolicitante = $idParam > 0
  ? 'ORDER BY CASE WHEN s.id = :id_order THEN 0 ELSE 1 END, s.id DESC'
  : 'ORDER BY s.id DESC';
if ($idParam > 0) {
  $paramsSolicitante[':id_order'] = $idParam;
}
...
$stmt = $pdo->prepare($sql);
$stmt->execute($paramsSolicitante);
...
$cpfParam = only_digits((string)($S['cpf'] ?? $cpfParam));
ensure_cadastro_solicitacoes($pdo, (int)$S['id'], $cpfParam);
...
/* Solicitação cadastral e solicitações adicionais */
$principalTipoId = isset($S['ajuda_tipo_id']) ? (int)$S['ajuda_tipo_id'] : 0;
$resumoCadastral = trim((string)($S['resumo_caso'] ?? ''));
$dataCadastroBase = fmt_date_short((string)($S['created_at'] ?? ''));
$solicitacaoCadastral = null;
$solicitacaoCadastralId = 0;
$SOLS = [];

try {
  if (table_exists($pdo, 'solicitacoes')) {
    $whereSolic = ['sol.solicitante_id = :sid'];
    $paramsSolic = [':sid' => (int)$S['id']];
    if (strlen($cpfParam) === 11) {
      $whereSolic[] = 'p.cpf = :cpf_solic';
      $paramsSolic[':cpf_solic'] = $cpfParam;
    }
    ...
    $stmtSolicitacoes = $pdo->prepare("
      ...
      LEFT JOIN solicitantes p ON p.id = sol.solicitante_id
      LEFT JOIN ajudas_tipos at ON at.id = sol.ajuda_tipo_id
      WHERE (" . implode(' OR ', $whereSolic) . ")
        AND COALESCE(sol.origem, '') <> 'cadastro_duplicada'
      ORDER BY sol.data_solicitacao ASC, sol.id ASC
    ");
    $stmtSolicitacoes->execute($paramsSolic);
    $todasSolicitacoes = $stmtSolicitacoes->fetchAll(PDO::FETCH_ASSOC) ?: [];

    $melhorScore = -1;
    foreach ($todasSolicitacoes as $sol) {
      if ((int)($sol['solicitante_id'] ?? 0) !== (int)$S['id']) {
        continue;
      }
      if (strtolower((string)($sol['origem'] ?? '')) === 'cadastro') {
        $solicitacaoCadastral = $sol;
        $melhorScore = 999;
        break;
      }
      $score = 0;
      $solTipoId = isset($sol['ajuda_tipo_id']) ? (int)$sol['ajuda_tipo_id'] : 0;
      $solResumo = trim((string)($sol['resumo_caso'] ?? ''));
      $solData = fmt_date_short((string)($sol['data_solicitacao'] ?? ''));

      if ($principalTipoId > 0 && $solTipoId === $principalTipoId) $score += 4;
      if ($resumoCadastral !== '' && $solResumo === $resumoCadastral) $score += 3;
      if ($dataCadastroBase !== '' && $solData === $dataCadastroBase) $score += 2;
    ...
    if ($solicitacaoCadastral && $melhorScore > 0) {
      $solicitacaoCadastralId = (int)$solicitacaoCadastral['id'];
    } else {
      $solicitacaoCadastral = null;
    }

    foreach ($todasSolicitacoes as $sol) {
      if ($solicitacaoCadastralId > 0 && (int)$sol['id'] === $solicitacaoCadastralId) {
        continue;
      }
      $SOLS[] = $sol;
    }

    usort($SOLS, static function (array $a, array $b): int {
      $dt = strcmp((string)($b['data_solicitacao'] ?? ''), (string)($a['data_solicitacao'] ?? ''));
      if ($dt !== 0) return $dt;
...
/* Status do caso cadastral: por solicitacao_id quando existir */
$infoStatus = status_por_entrega(
  $pdo,
  $cpfParam,
  $principalTipoId ?: null,
  $solicitacaoCadastralId ?: null,
  (int)$S['id']
);
    ...
    <div class="sheet">
      ...
      <div class="hdr">
        ...
        <?php if ($fotoSolicitanteSrc !== ''): ?>
          <div class="photo-3x4" title="Foto 3x4 do solicitante">
            <img src="<?= h($fotoSolicitanteSrc) ?>" alt="Foto 3x4 do solicitante" onerror="this.onerror=null;this.closest('.photo-3x4').classList.add('photo-empty');this.src='assets/images/logo/logo_pmc_2025.jpg';">
          </div>
        <?php else: ?>
      ...
      <div class="box">
        <div class="cap cap-flex">
          ...
          $statusEntregaV = (string)($entV['status'] ?? 'ABERTO');
          $statusV = $statusEntregaV === 'CONCLUÍDO'
            ? $statusEntregaV
            : (string)($solicitacaoCadastral['status'] ?? 'ABERTO');
          $entregueV = (string)($entV['entregue_em'] ?? ($entV['data'] ?? ''));
          $valorEntregaV = (string)($entV['valor_folha'] ?? '');
          ?>
          ...
          <?php if (!empty($SOLS)): ?>
            <?php foreach ($SOLS as $sol): ?>
              ...
              $entInfo = status_por_entrega(
                ...
                isset($sol['id']) ? (int)$sol['id'] : null,
                isset($sol['solicitante_id']) ? (int)$sol['solicitante_id'] : (int)$S['id']
              );

              $statusEntrega = (string)($entInfo['status'] ?? 'ABERTO');
              $statusExibir = $statusEntrega === 'CONCLUÍDO'
                ? $statusEntrega
                : (string)($sol['status'] ?? 'ABERTO');
              $dataEntregaSol = (string)($entInfo['entregue_em'] ?? ($entInfo['data'] ?? ''));
              $valorEntregaSol = (string)($entInfo['valor_folha'] ?? '');
              ?>
              <div class="ruled">
                ...
                <div class="sol-status-line">
                  ...
                  <?php endif; ?>
                  <?php if ($valorEntregaSol !== ''): ?>
                    <div> &nbsp;|&nbsp; <span><b><?= h($valorEntregaSol) ?></b></span></div>
                  <?php endif; ?>
                </div>
              </div>
            <?php endforeach; ?>
