...
// Ajuste o caminho da conexão conforme seu projeto
require_once __DIR__ . '/../assets/conexao.php';

// (Opcional) garantir fuso horário coerente
...
try {
    ...
    // 1) Verifica se o e-mail existe na tabela de contas
    ...
    // 2) Invalida tokens anteriores ainda não usados (opcional, para manter apenas 1 válido)
    $pdo->prepare("UPDATE senha_tokens SET used = 1 WHERE email = :email AND used = 0")
        ->execute([':email' => $email]);

    // 3) Gera novo código e define validade (3 minutos)
    //    Se quiser permitir zero à esquerda, use str_pad(random_int(0,999999), 6, '0', STR_PAD_LEFT)
    ...
    // 5) Monta e envia o e-mail HTML (mesmo estilo que você usou)
    ...
    <body>
        <div class='container'>
            ...
            <p>Recebemos uma solicitação para redefinir sua senha no sistema da<strong>SEMAS</strong>.</p>
            <p>Utilize o código abaixo para prosseguir. Ele é válido por <strong>3 minutos</strong>:</p>
    ...
    if (!$ok) {
        // Se falhar envio, você pode manter o token (para tentar reenviar) ou invalidar.
        echo "<script>alert('Erro ao enviar o e-mail. Tente novamente.'); history.back();</script>";
        exit;
    }

    // 6) Sucesso: alerta e redireciona para verificação do código
    echo "<script>
