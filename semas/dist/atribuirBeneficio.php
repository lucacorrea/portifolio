...
if (($_GET['searchPessoa'] ?? '') === '1') {
    ...
    $sql = "SELECT id, nome, cpf, endereco, numero, telefone
            FROM solicitantes
            WHERE (nome LIKE :q OR cpf LIKE :cpfq OR telefone LIKE :telq OR id = :qid)
            ORDER BY nome ASC
            LIMIT 10";
    $st = $pdo->prepare($sql);
    $st->bindValue(':q', "%{$q}%");
    $st->bindValue(':cpfq', "%{$qDigits}%");
    $st->bindValue(':telq', "%{$qDigits}%");
    $st->bindValue(':qid', $qDigits !== '' ? (int)$qDigits : 0, PDO::PARAM_INT);
    $st->execute();
    ...
    try {
        ...
        $entregaFields = $hasSolicitacaoEntrega
            ? "COALESCE(ent.entregas_count, 0) AS entregas_count,
               ent.data_entrega AS ultima_entrega_data,
               ent.hora_entrega AS ultima_entrega_hora"
            : "0 AS entregas_count,
               NULL AS ultima_entrega_data,
               NULL AS ultima_entrega_hora";
        ...
        $st = $pdo->prepare("
            ...
            INNER JOIN solicitantes p ON p.id = sol.solicitante_id
            LEFT JOIN ajudas_tipos at ON at.id = sol.ajuda_tipo_id
            {$entregaJoin}
            WHERE sol.id = :id
            LIMIT 1
        ");
        $st->execute([':id' => $solicitacaoId]);
        $prefSolicitacao = $st->fetch(PDO::FETCH_ASSOC) ?: null;

        if ($prefSolicitacao) {
            $prefPessoa = [
                'id' => (int)$prefSolicitacao['pessoa_id'],
                'nome' => (string)$prefSolicitacao['pessoa_nome'],
                'cpf' => (string)$prefSolicitacao['pessoa_cpf'],
                'endereco' => (string)($prefSolicitacao['pessoa_endereco'] ?? ''),
                'numero' => (string)($prefSolicitacao['pessoa_numero'] ?? ''),
                'telefone' => (string)($prefSolicitacao['pessoa_telefone'] ?? ''),
            ];
            $cpfUrl = only_digits((string)$prefSolicitacao['pessoa_cpf']);
        }
    } catch (Throwable $e) {
        $prefSolicitacao = null;
    }
...
if ($cpfUrl !== '' && strlen($cpfUrl) === 11) {
    ...
    $prefPessoa = $prefPessoa ?: ($st->fetch(PDO::FETCH_ASSOC) ?: null);
}
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<body>
                        ...
                        <div class="card-body">
                            ...
                            <form method="post" action="./dados/processarBeneficioSolicitante.php" class="form-grid">
                                ...
                                <div class="field col-span-2-md">
                                    <label class="form-label">Solicitação que será atendida</label>
                                    <select name="solicitacao_id" id="solicitacao_id" class="form-select" required>
                                        <option value="">Selecione o solicitante primeiro...</option>
                                    </select>
                                    <div class="help" id="infoSolicitacao">
                                        A solicitação é separada por data/hora para diferenciar pedidos iguais da mesma pessoa.
                                    </div>
                                </div>
                                    ...
                                    <select name="ajuda_tipo_id" id="ajuda_tipo_id" class="form-select" required>
                                        ...
                                        <?php foreach ($tipos as $t): ?>
                                            <option value="<?= (int) $t['id'] ?>"
                                                ...
                                                <?= ($prefSolicitacao && (int)($prefSolicitacao['ajuda_tipo_id'] ?? 0) === (int)$t['id']) ? 'selected' : '' ?>>
    ...
    <script>
        ...
        const $solicitacao = document.getElementById('solicitacao_id');
        const $infoSolicitacao = document.getElementById('infoSolicitacao');
        const initialPessoaId = <?= $prefPessoa ? (int)$prefPessoa['id'] : 0 ?>;
        const initialSolicitacaoId = <?= (int)$solicitacaoId ?>;
        ...
        function setPessoa(item) {
            ...
            $box.innerHTML = '';
            carregarSolicitacoes(item.id, '');
        }

        function setSolicitacaoLoading(text) {
            $solicitacao.innerHTML = `<option value="">${escapeHtml(text)}</option>`;
            $infoSolicitacao.textContent = text;
        }

        function renderSolicitacoesOptions(items, selectedId) {
            solicitacoesCache = Array.isArray(items) ? items : [];
            $solicitacao.innerHTML = '';

            if (!solicitacoesCache.length) {
                setSolicitacaoLoading('Nenhuma solicitação registrada. Crie uma solicitação antes de atribuir o benefício.');
                return;
            }

            const placeholder = document.createElement('option');
            placeholder.value = '';
            placeholder.textContent = 'Selecione uma solicitação...';
            $solicitacao.appendChild(placeholder);

            solicitacoesCache.forEach(item => {
                const opt = document.createElement('option');
                const entregasCount = Number(item.entregas_count || 0);
                const resumo = String(item.resumo_caso || '').trim();
                const resumoCurto = resumo.length > 70 ? resumo.slice(0, 70) + '...' : resumo;
                const pessoaRef = item.solicitante_id ? `Pessoa #${item.solicitante_id} — ` : '';
                opt.value = String(item.id);
                opt.dataset.ajudaTipoId = String(item.ajuda_tipo_id || '');
                opt.dataset.resumo = resumo;
                opt.textContent = `#${item.id} — ${pessoaRef}${item.data_fmt || 'sem data'} — ${item.ajuda_nome || 'Não informado'}${resumoCurto ? ' — ' + resumoCurto : ''}${entregasCount > 0 ? ` — ${entregasCount} entrega(s)` : ''}`;
                $solicitacao.appendChild(opt);
            });

            if (selectedId) {
                $solicitacao.value = String(selectedId);
            }

            if (!$solicitacao.value && solicitacoesCache.length === 1) {
                $solicitacao.value = String(solicitacoesCache[0].id);
            }

            atualizarSolicitacaoSelecionada();
        }

        async function carregarSolicitacoes(pessoaId, selectedId) {
            if (!pessoaId) {
                setSolicitacaoLoading('Selecione o solicitante primeiro...');
                return;
            }

            setSolicitacaoLoading('Carregando solicitações...');

            try {
                const url = new URL('api/get_solicitacoes.php', window.location.href);
                url.searchParams.set('solicitante_id', String(pessoaId));
                const cpfDigits = String($cpfH.value || '').replace(/\D+/g, '');
                ...
                if (!j.ok) {
                    setSolicitacaoLoading('Não foi possível carregar as solicitações.');
                    return;
                }

                renderSolicitacoesOptions(j.items || [], selectedId);
            } catch (e) {
                setSolicitacaoLoading('Falha ao carregar solicitações.');
            }
        }

        function atualizarSolicitacaoSelecionada() {
            const selected = solicitacoesCache.find(item => String(item.id) === String($solicitacao.value));
            if (!selected) {
                $infoSolicitacao.textContent = 'Escolha exatamente qual solicitação será atendida.';
                return;
            }

            const ajudaTipoId = String(selected.ajuda_tipo_id || '');
            if (ajudaTipoId && selAjuda.value !== ajudaTipoId) {
                selAjuda.value = ajudaTipoId;
                selAjuda.dispatchEvent(new Event('change'));
            }

            const entregasCount = Number(selected.entregas_count || 0);
            $infoSolicitacao.textContent = `Solicitação #${selected.id} registrada em ${selected.data_fmt || 'data não informada'}${entregasCount > 0 ? `; já possui ${entregasCount} entrega(s) registrada(s).` : '; ainda sem entrega registrada.'}`;
        }
        ...
        $solicitacao.addEventListener('change', atualizarSolicitacaoSelecionada);

        if (initialPessoaId > 0) {
            carregarSolicitacoes(initialPessoaId, initialSolicitacaoId > 0 ? initialSolicitacaoId : '');
        } else {
            setSolicitacaoLoading('Selecione o solicitante primeiro...');
        }
