...
function ensure_solicitacoes_table(PDO $pdo): void
{
    try {
        ...
        alert_back('Não foi possível preparar a tabela de solicitações.');
    }
}
...
/* ===== NOVO: Auxílio (ajudas_tipos) =====
   Esperado do select: name="categoria_entrevista"
*/
...
$resumo_caso = ($resumo_caso !== null && trim((string) $resumo_caso) !== '') ? trim((string) $resumo_caso) : null;
...
try {
    ...
    if ($ajuda_tipo_id !== null || $resumo_caso !== null) {
        $stmtSolicitacao = $pdo->prepare("
            INSERT INTO solicitacoes
                (solicitante_id, ajuda_tipo_id, resumo_caso, data_solicitacao, status, created_by, origem)
            VALUES
                (:solicitante_id, :ajuda_tipo_id, :resumo_caso, :data_solicitacao, 'Aberto', :created_by, 'cadastro')
        ");
        $stmtSolicitacao->execute([
            ':solicitante_id' => $solicitante_id,
            ':ajuda_tipo_id' => $ajuda_tipo_id,
            ':resumo_caso' => $resumo_caso,
            ':data_solicitacao' => $hora_cadastro ?: date('Y-m-d H:i:s'),
            ':created_by' => $responsavel ?: null,
        ]);
    }
