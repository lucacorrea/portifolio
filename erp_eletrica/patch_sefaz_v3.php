...
class FiscalService extends BaseService {
    ...
    public function testConnection($branchId) {
        $branch = $this->getBranchData($branchId);
        if (empty($branch['certificado_pfx'])) throw new Exception("Certificado Digital ausente.");
        ...
        try {
            ...
            if (!openssl_pkcs12_read($pfxContent, $certs, $password)) {
                throw new Exception("Falha ao ler o certificado digital.");
            }
    ...
    // Simplificando o arquivo no patcher para manter as funções base, focar só no testConnection
...
// Update apenas as funções críticas de FiscalService sem remover as de emissão inteiras
$fiscalContent = file_get_contents(__DIR__ . '/src/App/Services/FiscalService.php');

// Inject error reporting na função testConnection
$newFunction = <<< 'EOT'
    public function testConnection($branchId) {
        $branch = $this->getBranchData($branchId);
        
        if (empty($branch['cnpj'])) throw new Exception("CNPJ não configurado para esta filial.");
        if (empty($branch['certificado_pfx'])) throw new Exception("Certificado Digital (.pfx) não enviado.");
        if (empty($branch['certificado_senha'])) throw new Exception("Senha do certificado não configurada.");
        ...
        try {
            ...
            try {
                ...
                // Parser com robustez máxima a namespaces e fallback
                $cleanXml = preg_replace('/(<\/?)(\w+):([^>]*>)/', '$1$3', $responseXml);
                $res = @simplexml_load_string($cleanXml);
                ...
                // Tentar diferentes caminhos dependendo do envelope da SEFAZ
                $nodes = $res->xpath('//retConsStatServ');
                if (empty($nodes)) {
                ...
                if (!$retStatus) {
                     throw new Exception("Estrutura retConsStatServ não encontrada.\n\nRAW XML RECEBIDO DA SEFAZ (Envie isso para IA):\n\n" . htmlspecialchars(substr($responseXml, 0, 1500)));
                }
...
// Faz o replace mágico na função usando RegExp para não apagar o resto da classe original 
$pattern = '/\s+public function testConnection\(\$branchId\) \{.*?(?=\s+private function saveFiscalRecord|\s+\})/s';
$fiscalContent = preg_replace($pattern, "\n" . $newFunction . "\n", $fiscalContent);

file_put_contents(__DIR__ . '/src/App/Services/FiscalService.php', $fiscalContent);
...
echo "<h1>✅ ARQUIVOS DA SEFAZ V3 ATUALIZADOS!</h1>";
echo "<p>Agora ele vai nos mostrar a cara verdadeira do erro (O XML Cru da Sefaz).</p>";
