...
try {
    product_remove_image($productId, (int) ($_POST['image_id'] ?? 0));
    header('Location: ' . site_url('admin/produto-form.php?id=' . $productId . '&success=imagem_removida'));
    exit;
} catch (InvalidArgumentException $exception) {
    error_log('[ArteFlor][product-image-remove-validation] ' . $exception->getMessage());
    header('Location: ' . $withMessage('error=imagem_nao_encontrada'));
    exit;
} catch (Throwable $exception) {
    error_log('[ArteFlor][product-image-remove] ' . $exception->getMessage());
