...
$compactAction = preg_replace('/\s+/', '', $action);
$compactRepository = preg_replace('/\s+/', '', $repository);
productDeletionAssert(
    is_string($compactPage) && is_string($compactAction) && is_string($compactRepository),
    'Fontes devem permitir inspeção estrutural.'
);

productDeletionAssert(str_contains($compactPage, "can('produto.excluir')"), 'Interface deve respeitar a permissão produto.excluir.');
...
productDeletionAssert(str_contains($compactAction, "product_action_context('produto.excluir')"), 'Servidor deve exigir permissão de exclusão.');
...
productDeletionAssert(str_contains($compactRepository, 'SELECTid,estoque,excluido_em') && str_contains($repository, 'FOR UPDATE'), 'Saldo deve ser validado com bloqueio transacional.');
...
productDeletionAssert(str_contains($compactRepository, 'excluido_em=CURRENT_TIMESTAMP'), 'Exclusão deve preservar auditoria temporal.');
productDeletionAssert(!str_contains(strtoupper($repository), 'DELETE FROM PRODUTOS'), 'Produto nunca deve ser apagado fisicamente.');
productDeletionAssert(str_contains($compactRepository, "\$sql='SELECTCOUNT(*)FROMprodutosWHEREcodigo_barras=:barcode'"), 'Código de barras excluído deve continuar reservado no histórico.');
