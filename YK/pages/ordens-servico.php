                ...
                <table class="os-table service-orders-table">
                    <thead>
                        <tr>
                            ...
                            <th>Local</th>
                            <th>Funcionarios</th>
                            <th>Data do servico</th><?php if ($canViewValues): ?><th>Valor total</th><?php endif; ?><th>Status</th>
                            <th>Acoes</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($orders as $order): ?>
                            ...
                            /*
                             * Natureza fiscal da OS
                             *
                             * - produto  -> NF-e/NFC-e
                             * - servico  -> NFS-e
                             * - outro    -> não é fiscalizado automaticamente; precisa ser classificado
                             *
                             * Usamos os subtotais já materializados na própria OS para evitar
                             * consultas N+1 ao banco apenas para decidir quais ações exibir.
                             */
                            ...
                            /*
                             * Chave por modelo + ambiente.
                             *
                             * O código anterior olhava somente o ambiente. Isso podia fazer,
                             * por exemplo, uma NFC-e em homologação esconder indevidamente
                             * a opção de NF-e em homologação.
                             */
                            $orderFiscalDocumentKeys = [];

                            foreach ($orderFiscalDocuments as $document) {
                                $documentModel = (string) ($document['modelo'] ?? '');
                                $documentEnvironment = (string) ($document['ambiente'] ?? '');

                                if ($documentModel !== '' && $documentEnvironment !== '') {
                                    $orderFiscalDocumentKeys[$documentModel . '|' . $documentEnvironment] = true;
                                }
                            }
                            ?>
                            <tr>
                                    ...
                                    <div class="dropdown table-action-dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            ...
                                            <?php if ($order->status() === 'finalizada'): ?>
                                                ...
                                                <?php if ($orderHasProducts): ?>

                                                    <li>
                                                        <h6 class="dropdown-header">
                                                            <i class="bi bi-box-seam me-1"></i>
                                                            Fiscal de produtos / peças
                                                        </h6>
                                                    </li>
                                                    ...
                                                    <?php if ($canIssueFiscal): ?>
                                                        ...
                                                        <?php foreach (['55' => 'NF-e de peças', '65' => 'NFC-e de peças'] as $fiscalModel => $fiscalLabel): ?>
                                                            ...
                                                            <?php
                                                            $homologationKey = $fiscalModel . '|homologacao';
                                                            $productionKey = $fiscalModel . '|producao';
                                                            ?>

                                                            <?php if (!isset($orderFiscalDocumentKeys[$homologationKey])): ?>
                                                                <li>
                                                                    <form
                                                                        method="post"
                                                            ...
                                                            <?php if (
                                                                ...
                                                                && !isset($orderFiscalDocumentKeys[$productionKey])
                                                            ): ?>
                                                                <li>
                                                                    <form
                                                                        method="post"
                                                    ...
                                                    <?php else: ?>
                                                        ...
                                                        <li>
                                                            <span class="dropdown-item-text text-muted">
                                                                ...
                                                                Sem permissão para emitir NF-e/NFC-e de peças.
                                                            </span>
                                                        </li>

                                                    <?php endif; ?>

                                                <?php elseif ($canIssueFiscal): ?>

                                                    <li>
                                                        <span
                                                            class="dropdown-item-text text-muted"
                                                            title="NF-e e NFC-e são destinadas aos itens classificados como produto/peça nesta OS."
                                                        >
                                                            <i class="bi bi-info-circle"></i>
                                                            NF-e/NFC-e não se aplica: OS sem peças/produtos.
                                                        </span>
                                                    </li>
                                                ...
                                                <?php if ($orderHasServices): ?>
                                                    ...
                                                    <?php if ($canIssueNfse): ?>
                                                        ...
                                                        <li>
                                                            <form
                                                                method="post"
                                                                action="actions/nfse-preparar.php"
                                                            >
                                                                <?= $csrf->field() ?>
                                                                <?php return_to_field(); ?>
                                                    ...
                                                    <?php else: ?>
                                                        ...
                                                        <li>
                                                            <span class="dropdown-item-text text-muted">
                                                                <i class="bi bi-info-circle"></i>
                                                                Serviços exigem o fluxo NFS-e.
                                                                A emissão NFS-e não está liberada para este usuário.
                                                            </span>
                                                        </li>
                                                ...
                                                <?php if ($orderHasOthers): ?>
                                                    ...
                                                    <li>
                                                        <span class="dropdown-item-text text-warning">
                                                            ...
                                                            Há itens “Outros”. Classifique-os como produto ou serviço
                                                            antes de incluí-los em documento fiscal.
                                                        </span>
                                                    </li>
                                            ...
                                            <?php if ($isOrderPaid): ?>
                                                ...
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                            <?php if ($canReverse && $order->status() === 'finalizada'): ?><li>
                                                    <hr class="dropdown-divider">
                                                </li>
                                                <li><button class="dropdown-item text-danger js-os-reverse" type="button" data-order-id="<?= h((string) $order->id()) ?>" data-order-number="<?= h($order->displayNumber()) ?>" data-bs-toggle="modal" data-bs-target="#modal-os-reverse"><i class="bi bi-arrow-counterclockwise"></i> Estornar OS</button></li><?php endif; ?>
...
<div class="modal fade" id="modal-os" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <form class="modal-content visual-modal js-os-form" method="post" action="actions/os-salvar.php" autocomplete="off">
            <div class="modal-header">
                <div>
                    <h2 class="modal-title fs-5" id="modal-os-title">Nova OS</h2>
                    <p class="text-muted small mb-0">Valores e itens sao recalculados no servidor.</p>
                </div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
            </div>
            <div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="os-id">
                <div class="alert alert-danger <?= $recovery === null ? 'd-none' : '' ?>" data-os-form-error role="alert"><?= h($recovery['error'] ?? '') ?></div>
                <ul class="nav nav-tabs mb-3" role="tablist">
                    <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#os-tab-client" type="button">Cliente</button></li>
                    <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#os-tab-items" type="button">Itens</button></li><?php if ($canTeam || $canSchedule): ?><li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#os-tab-team" type="button">Equipe</button></li><?php endif; ?><li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#os-tab-values" type="button">Valores</button></li>
                </ul>
                <div class="tab-content">
                    <div class="tab-pane fade show active" id="os-tab-client">
                        <section class="form-section">
                            <div class="form-row">
                                <div class="form-group"><label class="form-label">Origem</label><select class="form-control-os" name="creation_mode" id="os-creation-mode">
                                        <option value="manual">Criar manualmente</option><?php if ($canConvertBudget): ?><option value="budget">Criar a partir de orcamento aprovado</option><?php endif; ?>
                                    </select></div>
                                <div class="form-group"><label class="form-label">Cliente</label><select class="form-control-os" name="client_id" id="os-client" required>
                                        <option value="">Selecione</option><?php os_select_options($clientOptions, '', true); ?>
                                    </select></div>
                                <div class="form-group"><label class="form-label">Orcamento aprovado</label><select class="form-control-os" name="budget_id" id="os-budget-id">
                                        <option value="">Selecione</option><?php foreach ($availableBudgets as $budget): ?><option value="<?= h((string) $budget['id']) ?>" data-client-id="<?= h((string) $budget['cliente_id']) ?>" data-total="<?= h((string) $budget['total']) ?>" data-summary="<?= h((string) ($budget['servicos_resumo'] ?? '')) ?>"><?= h(($budget['numero'] ?? sprintf('ORC-%06d', (int) $budget['id'])) . ' - ' . $budget['cliente_nome'] . ' - ' . os_money_fmt((string) $budget['total'])) ?></option><?php endforeach; ?>
                                    </select></div>
                                <div class="form-group"><label class="form-label">Status inicial</label><select class="form-control-os" name="status" id="os-status">
                            ...
                            <div class="form-row">
                                <div class="form-group"><label class="form-label">Prioridade</label><select class="form-control-os" name="priority" id="os-priority">
                                        <option value="baixa">Baixa</option>
                                        <option value="media" selected>Media</option>
                                        <option value="alta">Alta</option>
                                        <option value="urgente">Urgente</option>
                                    </select></div>
                                <div class="form-group"><label class="form-label">Tipo do equipamento</label><input class="form-control-os" name="equipment_type" id="os-equipment-type"></div>
                                <div class="form-group"><label class="form-label">Marca</label><input class="form-control-os" name="equipment_brand" id="os-equipment-brand"></div>
                                <div class="form-group"><label class="form-label">Modelo</label><input class="form-control-os" name="equipment_model" id="os-equipment-model"></div>
                            </div>
                            <div class="form-row">
                                <div class="form-group"><label class="form-label">Capacidade</label><input class="form-control-os" name="equipment_capacity" id="os-equipment-capacity"></div>
                                <div class="form-group"><label class="form-label">Numero de serie</label><input class="form-control-os" name="equipment_serial_number" id="os-equipment-serial-number"></div>
                                <div class="form-group"><label class="form-label">Ambiente</label><input class="form-control-os" name="equipment_environment" id="os-equipment-environment"></div>
                                <div class="form-group"><label class="form-label">Local</label><input class="form-control-os" name="equipment_location" id="os-equipment-location"></div>
                            </div>
                            <div class="alert alert-info d-none" id="os-budget-preview"></div>
                        </section>
                        <section class="form-section">
                            <h3 class="form-section-title">Diagnostico e observacoes</h3>
                            <div class="form-row">
                                <div class="form-group"><label class="form-label">Problema relatado</label><textarea class="form-control-os" name="reported_problem" id="os-reported-problem"></textarea></div>
                                <div class="form-group"><label class="form-label">Problema identificado</label><textarea class="form-control-os" name="identified_problem" id="os-identified-problem"></textarea></div>
                            </div>
                            <div class="form-row">
                                <div class="form-group"><label class="form-label">Diagnostico</label><textarea class="form-control-os" name="diagnosis" id="os-diagnosis"></textarea></div>
                                <div class="form-group"><label class="form-label">Solucao</label><textarea class="form-control-os" name="solution" id="os-solution"></textarea></div>
                            </div>
                            <div class="form-row">
                                <div class="form-group"><label class="form-label">Recomendacao</label><textarea class="form-control-os" name="recommendation" id="os-recommendation"></textarea></div>
                                <div class="form-group"><label class="form-label">Observacoes internas</label><textarea class="form-control-os" name="internal_notes" id="os-internal-notes"></textarea></div>
                            </div>
                        </section>
                    </div>
                    ...
                    <div class="tab-pane fade" id="os-tab-values">
                        <section class="form-section">
                            <div class="form-row">
                                <div class="form-group"><label class="form-label">Desconto</label><input class="form-control-os js-os-discount" name="discount" id="os-discount" value="0,00"></div>
                                <div class="form-group"><label class="form-label">Acrescimo</label><input class="form-control-os js-os-increase" name="increase" id="os-increase" value="0,00"></div>
                            </div><?php if ($canViewValues): ?><div class="summary-box js-os-summary">
                                    <div><span>Servicos</span><strong data-summary="servico">R$ 0,00</strong></div>
                                    <div><span>Produtos</span><strong data-summary="produto">R$ 0,00</strong></div>
                                    <div><span>Outros</span><strong data-summary="outro">R$ 0,00</strong></div>
                                    <div class="total"><span>Total</span><strong data-summary="total">R$ 0,00</strong></div>
                                </div><?php endif; ?><div class="form-group"><label class="form-label">Observacoes externas</label><textarea class="form-control-os" name="notes" id="os-notes"></textarea></div>
                        </section>
                    </div>
                </div>
            </div>
...
<template id="os-item-template">
    <div class="form-row os-item-row"><input type="hidden" data-field="id"><input type="hidden" data-field="type"><input type="hidden" data-field="origin" value="manual"><input type="hidden" data-field="budget_item_id">
        ...
        <div class="form-group"><label class="form-label">Desconto</label><input class="form-control-os" data-field="discount" value="0,00"></div>
        <div class="form-group"><label class="form-label">Subtotal</label><input class="form-control-os" data-field="subtotal" readonly></div>
        <div class="form-group"><label class="form-label">&nbsp;</label><button class="btn-filter btn-filter-ghost js-os-remove-item" type="button"><i class="bi bi-trash"></i></button></div>
    </div>
</template>
<template id="os-team-member-template">
    <div class="form-row os-team-member-row">
        <div class="form-group"><label class="form-label">Funcionário</label><select class="form-control-os" data-team-field="funcionario_id">
                <option value="">Selecione</option>
            </select></div>
        <div class="form-group"><label class="form-label">Função</label><select class="form-control-os" data-team-field="funcao">
        ...
        <div class="form-group"><label class="form-label">Principal</label>
            <div class="form-check mt-2"><input class="form-check-input" type="radio" data-team-field="principal" value="1"><span class="form-check-label">Responsável</span></div>
        </div>
        <div class="form-group"><label class="form-label">&nbsp;</label><button class="btn-filter btn-filter-ghost js-os-remove-team-member" type="button" aria-label="Remover funcionário"><i class="bi bi-trash"></i></button></div>
    </div>
</template>

<div class="modal fade" id="modal-os-view" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content visual-modal">
            <div class="modal-header">
                <div>
                    <h2 class="modal-title fs-5">Dados da OS</h2>
                    <p class="text-muted small mb-0" id="os-view-subtitle"></p>
                </div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
            </div>
            <div class="modal-body">
                <section class="form-section">
                    <h3 class="form-section-title">Resumo</h3>
                    <div class="summary-box" id="os-view-summary"></div>
                </section>
                <section class="form-section">
...
<div class="modal fade" id="modal-os-team" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <form class="modal-content visual-modal" method="post" action="actions/os-equipe-agendamento.php">
            <div class="modal-header">
...
<div
    class="modal fade"
    id="modal-os-finalize"
    tabindex="-1"
    aria-hidden="true"
    aria-labelledby="os-finalize-title">
    <div
        class="modal-dialog modal-xl modal-dialog-scrollable">
        <form
            class="modal-content visual-modal"
            method="post"
            action="actions/os-finalizar.php"
            autocomplete="off">
            <div class="modal-header">
                <div>
                    <h2
                        class="modal-title fs-5"
                        id="os-finalize-title">
                        Finalizar OS
                    </h2>

                    <p class="text-muted small mb-0">
                        Confirme os itens e o valor final da ordem
                        de serviço.
                    </p>
                </div>
            ...
            <div class="modal-body">
                ...
                <div
                    ...
                    Os serviços, produtos, quantidades e valores
                    serão carregados automaticamente da OS.

                    Após finalizar, o sistema perguntará se o
                    cliente já realizou o pagamento.
                </div>

                <div
                    class="alert alert-danger"
                    id="os-finalize-error"
                    role="alert"
                    hidden></div>

                <div
                    id="os-finalize-loading"
                    ...
                    <p class="text-muted mt-3 mb-0">
                        Carregando os dados da OS...
                    </p>
                </div>

                <div
                    id="os-finalize-content"
                    hidden>
                    <section class="form-section">
                        <h3 class="form-section-title">
                            Itens da
                            <span id="os-finalize-number"></span>
                        </h3>
                    ...
                    <section class="form-section">
                        ...
                        <div class="form-row mt-3">
                            <div class="form-group">
                                <label
                                    class="form-label"
                                    for="os-finalize-discount">
                                    Desconto geral
                                </label>

                                <input
                                    class="form-control-os"
                                    id="os-finalize-discount"
                                    name="desconto"
                                    value="0,00"
                                    inputmode="decimal"
                                    maxlength="20"
                            ...
                            <div class="form-group">
                                <label
                                    class="form-label"
                                    for="os-finalize-increase">
                                    Acréscimo geral
                                </label>

                                <input
                                    class="form-control-os"
                                    id="os-finalize-increase"
                                    name="acrescimo"
                                    value="0,00"
                                    inputmode="decimal"
                                    maxlength="20"
                        ...
                        <div
                            ...
                            <div
                                ...
                                <strong>Valor final da OS</strong>

                                <strong
                                    class="fs-4"
                                    id="os-finalize-total">
                                    R$ 0,00
                                </strong>
                            </div>
                        </div>
                    ...
                    <section class="form-section">
                        ...
                        <p class="text-muted small">
                            Os campos abaixo são opcionais. O
                            pagamento será informado na próxima
                            etapa.
                        </p>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label
                                    class="form-label"
                                    for="os-finalize-due-date">
                                    Vencimento
                                </label>

                                <input
                                    class="form-control-os"
                                    id="os-finalize-due-date"
                                    type="date"
                                    name="vencimento_em">
                            </div>
                            ...
                            <div class="form-group">
                                <label
                                    class="form-label"
                                    for="os-finalize-reminder-date">
                                    Próximo lembrete
                                </label>

                                <input
                                    class="form-control-os"
                                    id="os-finalize-reminder-date"
                                    type="date"
                        ...
                        <div class="form-group">
                            <label
                                class="form-label"
                                for="os-finalize-notes">
                                Observação da finalização
                            </label>

                            <textarea
                                class="form-control-os"
                                id="os-finalize-notes"
                                name="observacao"
                                maxlength="1000"
                                rows="3"></textarea>
                        </div>
                        ...
                        <div class="form-group">
                            <label
                                class="form-label"
                                for="os-finalize-balance-notes">
                                Observação da conta a receber
                            </label>

                            <textarea
                                class="form-control-os"
                                id="os-finalize-balance-notes"
                                name="saldo_observacao"
                                maxlength="1000"
                                rows="3"></textarea>
                        </div>
                    </section>
...
<?php if ($canPayOrder): ?><div class="modal fade" id="modal-os-pay" tabindex="-1" aria-hidden="true" aria-labelledby="os-pay-title" aria-describedby="os-pay-subtitle">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <form class="modal-content visual-modal" method="post" action="actions/os-pagar.php" autocomplete="off">
                <div class="modal-header">
                    <div>
                        <h2 class="modal-title fs-5" id="os-pay-title">Pagamento da OS</h2>
                        <p class="text-muted small mb-0" id="os-pay-subtitle">A conclusão já foi salva. Agora informe se o cliente pagou.</p>
                    </div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="os-pay-id"><input type="hidden" name="payment_token" id="os-pay-token">
                    <div class="alert alert-success" id="os-pay-completion" role="status" hidden></div>
                    <div id="os-pay-question" hidden>
                        <p class="fs-5 mb-2">O cliente já pagou este serviço?</p>
                        <p class="text-muted mb-0">Se ainda não pagou, a OS continuará concluída e o valor ficará pendente em Contas a Receber.</p>
                    </div>
                    <div id="os-pay-fields">
                        <div class="alert alert-info" id="os-pay-summary" role="status"></div>
                        <div class="alert alert-danger" id="os-pay-error" role="alert" hidden></div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label" for="os-pay-value">Valor recebido</label><input class="form-control-os" id="os-pay-value" name="valor" inputmode="decimal" required></div>
                            <div class="form-group"><label class="form-label" for="os-pay-method">Forma de pagamento</label><select class="form-control-os" id="os-pay-method" name="forma_pagamento" required>
                                    <option value="dinheiro">Dinheiro</option>
                                    <option value="pix">Pix</option>
                                    <option value="cartao_debito">Cartão de débito</option>
                                    <option value="cartao_credito">Cartão de crédito</option>
                                    <option value="boleto">Boleto já compensado</option>
                        ...
                        <div class="form-group mb-0"><label class="form-label" for="os-pay-notes">Observação</label><textarea class="form-control-os" id="os-pay-notes" name="observacao" maxlength="255" rows="3"></textarea></div>
                    </div>
                </div>
                <div class="modal-footer" id="os-pay-question-actions" hidden><button class="btn-modal-cancel" id="os-pay-leave-pending" type="button" data-bs-dismiss="modal">Não, deixar pendente</button><button class="btn-modal-save" id="os-pay-confirm-paid" type="button"><i class="bi bi-cash-coin"></i> Sim, registrar pagamento</button></div>
                <div class="modal-footer" id="os-pay-form-actions"><button class="btn-modal-cancel" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn-modal-save" type="submit"><i class="bi bi-cash-coin"></i> Confirmar pagamento e gerar recibo</button></div>
            </form>
        </div>
    </div>
    </div><?php endif; ?>
<div class="modal fade" id="modal-os-cancel" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <form class="modal-content visual-modal" method="post" action="actions/os-cancelar.php">
            <div class="modal-header">
                <h2 class="modal-title fs-5">Cancelar servico</h2><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
            </div>
            <div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="os-cancel-id">
                <div class="form-row">
                    <div class="form-group"><label class="form-label">Destino do orcamento</label><select class="form-control-os" name="opcao" required>
                            <option value="definitivo">Cancelar definitivamente</option>
                            <option value="liberar_orcamento">Cancelar e liberar o orcamento</option>
                            <option value="criar_substituta">Cancelar e criar OS substituta</option>
                        </select></div>
                    <div class="form-group"><label class="form-label">Motivo</label><input class="form-control-os" name="motivo" required></div>
                </div>
                <div class="form-group"><label class="form-label">Observacao</label><textarea class="form-control-os" name="observacao"></textarea></div>
            </div>
            <div class="modal-footer"><button class="btn-modal-cancel" type="button" data-bs-dismiss="modal">Voltar</button><button class="btn-modal-save" type="submit">Confirmar</button></div>
        </form>
    </div>
</div>
<div class="modal fade" id="modal-os-status" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content visual-modal" method="post" action="actions/os-status.php">
            <div class="modal-header">
                <h2 class="modal-title fs-5" id="os-status-title">Alterar status</h2><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
            </div>
            <div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="os-status-id"><input type="hidden" name="operation" id="os-status-operation">
                <p id="os-status-message"></p>
            </div>
            <div class="modal-footer"><button class="btn-modal-cancel" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn-modal-save" type="submit">Confirmar</button></div>
        </form>
    </div>
</div>

<?php if ($canReverse): ?><div class="modal fade" id="modal-os-reverse" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form class="modal-content visual-modal" method="post" action="actions/os-estornar.php">
                <div class="modal-header">
                    <h2 class="modal-title fs-5">Estornar OS</h2><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="os-reverse-id">
                    <p>O estorno da <strong id="os-reverse-number"></strong> desfará a finalização e criará lançamentos compensatórios de estoque e caixa, preservando o histórico.</p>
                    <div class="form-group"><label class="form-label" for="os-reverse-reason">Motivo do estorno</label><textarea class="form-control-os" id="os-reverse-reason" name="motivo" maxlength="255" required></textarea></div>
                </div>
                <div class="modal-footer"><button class="btn-modal-cancel" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn-modal-save" type="submit">Confirmar estorno</button></div>
            </form>
        </div>
    </div><?php endif; ?>
...
<?php if ($canIssueReceipt): ?><div class="modal fade" id="modal-os-receipt" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form class="modal-content visual-modal" method="post" action="actions/recibo-emitir.php" target="_blank">
                <div class="modal-header">
                    <h2 class="modal-title fs-5">Gerar recibo de pagamento</h2><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body"><?= $csrf->field() ?><input type="hidden" name="pagamento_id" id="os-receipt-payment-id">
                    <p>Gerar recibo da <strong id="os-receipt-order-number"></strong> referente a <strong id="os-receipt-payment-label"></strong>?</p>
                    <p class="text-muted small mb-0">Depois de gerar, escolha entre impressão térmica de 80 mm ou A4 em impressora comum.</p>
                </div>
                <div class="modal-footer"><button class="btn-modal-cancel" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn-modal-save" type="submit">Gerar e escolher impressão</button></div>
            </form>
        </div>
    </div><?php endif; ?>

<script type="application/json" id="os-page-data">
    <?= json_encode(['services' => $serviceOptions, 'products' => $productOptions, 'employees' => $employeeOptions, 'recoveryModal' => $recovery['modal'] ?? ($_GET['modal'] ?? null), 'recoveryData' => $recovery['data'] ?? [], 'recoveryError' => $recovery['error'] ?? null, 'postCompletionPaymentPrompt' => $canPayOrder ? $postCompletionPaymentPrompt : null], JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>
</script>
<script>
