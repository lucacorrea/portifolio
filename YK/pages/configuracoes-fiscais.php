        ...
        <?php if ($canConfigure && $certificates !== [] && $overview !== null): ?>
          <form method="post" action="actions/configuracao-fiscal-salvar.php">
            ...
            <?php if ($selectedModel === '65'): ?>
              <div class="alert alert-info py-2">QR Code v3 selecionado. CSC/ID são opcionais e só devem ser informados para compatibilidade com QR v2.</div>
              <div class="form-row"><div class="form-group"><label class="form-label" for="fiscal-csc-id">ID do CSC (somente QR v2)</label><input class="form-control-os" id="fiscal-csc-id" name="csc_id" maxlength="40" autocomplete="off"></div><div class="form-group"><label class="form-label" for="fiscal-csc">CSC de homologação (somente QR v2)</label><input class="form-control-os" id="fiscal-csc" type="password" name="csc" maxlength="120" autocomplete="new-password"></div></div>
