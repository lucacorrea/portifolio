                                ...
                                <?php foreach ($teamOrders as $order): ?>
                                    <article class="week-service-card priority-<?= h($order->priority()) ?>">
                                        ...
                                        <div class="mt-2 d-flex flex-wrap gap-2">
                                            <?php if ($canEdit): ?><a class="btn-filter btn-filter-ghost" href="ordens-servico.php?search=<?= h(rawurlencode($order->displayNumber())) ?>">OS</a><?php endif; ?>
                                            <?php if ($canSchedule): ?><button class="btn-filter btn-filter-ghost js-week-schedule" type="button" data-order-id="<?= h((string) $order->id()) ?>" data-start="<?= h($order->scheduledStart() ?? '') ?>" data-end="<?= h($order->scheduledEnd() ?? '') ?>" data-bs-toggle="modal" data-bs-target="#modal-week-schedule">Reagendar</button><?php endif; ?>
                                            <?php if ($canTeam): ?><button class="btn-filter btn-filter-ghost js-week-team" type="button" data-order-id="<?= h((string) $order->id()) ?>" data-primary-id="<?= h((string) ($order->primaryEmployeeId() ?? '')) ?>" data-support-id="<?= h((string) ($order->supportEmployeeId() ?? '')) ?>" data-bs-toggle="modal" data-bs-target="#modal-week-team">Dupla</button><?php endif; ?>
                                            <?php if ($canStatus): ?><button class="btn-filter btn-filter-ghost js-week-status" type="button" data-order-id="<?= h((string) $order->id()) ?>" data-bs-toggle="modal" data-bs-target="#modal-week-status">Status</button><?php endif; ?>
                                            <?php if ($canCancel && !in_array($order->status(), ['finalizada','cancelada'], true)): ?><form method="post" action="actions/painel-semanal-cancelar.php"><?= $csrf->field() ?><input type="hidden" name="id" value="<?= h((string) $order->id()) ?>"><button class="btn-filter btn-filter-ghost text-danger" type="submit">Cancelar</button></form><?php endif; ?>
                                        </div>
                                    </article>
                                <?php endforeach; ?>
                            </section>
