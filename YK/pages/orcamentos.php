...
$canDelete = $authorization->can('orcamento.excluir');
$recovery = budget_consume_form_recovery();

function budget_data(?array $recovery, string $modal): array
{
    return $recovery !== null && ($recovery['modal'] ?? '') === $modal && is_array($recovery['data'] ?? null) ? $recovery['data'] : [];
}
function budget_error(?array $recovery, string $modal): ?string
{
    return $recovery !== null && ($recovery['modal'] ?? '') === $modal && is_string($recovery['error'] ?? null) ? $recovery['error'] : null;
}
function budget_value(array $data, string $key, string $default = ''): string
{
    $value = $data[$key] ?? $default;
    return is_scalar($value) ? (string) $value : $default;
}
function budget_money(string $value): string
{
    return money($value);
}
function budget_date(string $value): string
{
    try {
...
{
    return ['rascunho' => 'Rascunho', 'enviado' => 'Enviado', 'aguardando_aprovacao' => 'Aguardando aprovação', 'aprovado' => 'Aprovado', 'recusado' => 'Recusado', 'vencido' => 'Vencido'][$status] ?? 'Rascunho';
}
function budget_status_class(string $status): string
{
    return ['rascunho' => 'gray', 'enviado' => 'blue', 'aguardando_aprovacao' => 'amber', 'aprovado' => 'green', 'recusado' => 'red', 'vencido' => 'red'][$status] ?? 'gray';
}
...
$createData = budget_data(
    $recovery,
    'create'
);

$editData = budget_data(
    $recovery,
    'edit'
);

$deleteData = budget_data(
    $recovery,
    'delete'
);

$createError = budget_error(
    $recovery,
    'create'
);

$editError = budget_error(
    $recovery,
    'edit'
);

$deleteError = budget_error(
    $recovery,
    'delete'
);
?>
...
<div class="page-body budgets-page">
    ...
    <form class="filter-bar" method="get" action="orcamentos.php" data-live-filter="budgets" data-live-regions="metrics results">
        ...
        <select class="filter-select" name="status" aria-label="Status">
            ...
            <option value="exceto_recusados" <?= $filters['status'] === 'exceto_recusados' ? 'selected' : '' ?>>Todos exceto canceladas</option><?php foreach (['rascunho', 'enviado', 'aguardando_aprovacao', 'aprovado', 'recusado', 'vencido'] as $status): ?><option value="<?= h($status) ?>" <?= $filters['status'] === $status ? 'selected' : '' ?>><?= h(budget_status_label($status)) ?></option><?php endforeach; ?>
        </select>
    ...
    <section class="panel" data-live-region="results">
        ...
        <div class="table-panel-wrap">
            <table class="os-table budgets-table">
                <thead>
                    <tr>
                        <th>Número</th>
                        <th>Cliente</th>
                ...
                <tbody>
                    <?php foreach ($budgets as $budget): ?>
                        ...
                        <tr>
                            ...
                            <td><?= h(budget_date($budget->issueDate())) ?></td>
                            <td><?= h(budget_date($budget->validUntil())) ?></td>
                            <td><?= h((string) $budget->itemsCount()) ?></td>
                            <td><?= h(budget_money($budget->total())) ?></td>
                            <td><span class="badge-soft badge-<?= h(budget_status_class($displayStatus)) ?>"><?= h(budget_status_label($displayStatus)) ?></span></td>
                            <td class="table-actions-cell">
                                <div class="dropdown table-action-dropdown"><button class="btn-action" type="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Ações do orçamento <?= h($budget->displayNumber()) ?>"><i class="bi bi-three-dots-vertical"></i></button>
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        ...
                                        <?php if ($canPrint): ?><li>
                                            ...
                                            <li><a class="dropdown-item" href="orcamento-imprimir.php?id=<?= h((string) $budget->id()) ?>" target="_blank" rel="noopener"><i class="bi bi-printer"></i> Imprimir / reimprimir orçamento</a></li><?php endif; ?>
                                        <?php if ($canDelete): ?>
                                            <?php
                                                                                                                                                            $linkedOrder = $ordersByBudget[$budget->id()] ?? null;
                                            ?>
                                            ...
                                            <li>
                                                <button
                                                    class="dropdown-item text-danger js-budget-delete"
                                                    type="button"
                                                    data-budget-id="<?= h((string) $budget->id()) ?>"
                                                    data-budget-number="<?= h($budget->displayNumber()) ?>"
                                                    data-linked-order="<?= h(
                                                                                                                                                                is_array($linkedOrder)
                                                                                                                                                                    ? (string) ($linkedOrder['numero'] ?? '')
                                                                                                                                                                    : ''
                                                                                                                                                            ) ?>"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#modal-orcamento-delete">
...
?>
    <div class="modal fade" id="<?= h($id) ?>" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <form class="modal-content visual-modal js-budget-form" method="post" action="actions/orcamento-salvar.php" autocomplete="off">
                <div class="modal-header">
                    <div>
                        <h2 class="modal-title fs-5"><?= h($title) ?></h2>
                        <p class="text-muted small mb-0"><?= $edit ? 'O número é imutável.' : 'O número será gerado automaticamente.' ?></p>
                    </div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body"><?= $GLOBALS['csrf']->field() ?><?php return_to_field(); ?><div class="alert alert-danger <?= $error === null ? 'd-none' : '' ?>" role="alert"><?= h($error ?? '') ?></div><?php if ($edit): ?><input type="hidden" name="id" id="edit-budget-id" value="<?= h(budget_value($data, 'id')) ?>">
                        <section class="form-section">
                            <h3 class="form-section-title">Número</h3><input class="form-control-os" id="edit-budget-number" value="<?= h(budget_value($data, 'number')) ?>" readonly>
                        </section><?php endif; ?><section class="form-section">
                        <h3 class="form-section-title">Dados do orçamento</h3>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Cliente</label><select class="form-control-os" name="client_id" id="<?= $edit ? 'edit' : 'create' ?>-budget-client" required>
                                    <option value="">Selecione</option><?php budget_select_options($clientOptions, budget_value($data, 'client_id'), !$edit); ?>
                                </select></div>
                            <div class="form-group"><label class="form-label">Data de emissão</label><input class="form-control-os" type="date" name="issue_date" id="<?= $edit ? 'edit' : 'create' ?>-budget-issue-date" value="<?= h(budget_value($data, 'issue_date', date('Y-m-d'))) ?>" required></div>
                            <div class="form-group"><label class="form-label">Validade</label><input class="form-control-os" type="date" name="valid_until" id="<?= $edit ? 'edit' : 'create' ?>-budget-valid-until" value="<?= h(budget_value($data, 'valid_until', date('Y-m-d', strtotime('+7 days')))) ?>" required></div>
                            <div class="form-group"><label class="form-label">Status</label><select class="form-control-os" name="status" id="<?= $edit ? 'edit' : 'create' ?>-budget-status">
                                    <option value="rascunho" <?= budget_value($data, 'status', 'rascunho') === 'rascunho' ? 'selected' : '' ?>>Rascunho</option>
                                    <option value="enviado" <?= budget_value($data, 'status') === 'enviado' ? 'selected' : '' ?>>Enviado</option>
                                    <option value="aguardando_aprovacao" <?= budget_value($data, 'status') === 'aguardando_aprovacao' ? 'selected' : '' ?>>Aguardando aprovação</option>
                                </select></div>
                        </div>
                    </section>
                    ...
                    <section class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Desconto geral</label><input class="form-control-os js-budget-discount" name="discount" value="<?= h(budget_value($data, 'discount', '0,00')) ?>"></div>
                            <div class="form-group"><label class="form-label">Acréscimo</label><input class="form-control-os js-budget-increase" name="increase" value="<?= h(budget_value($data, 'increase', '0,00')) ?>"></div>
                        </div>
                        <div class="summary-box js-budget-summary">
                            <div><span>Subtotal de serviços</span><strong data-summary="servico">R$ 0,00</strong></div>
                            <div><span>Subtotal de produtos</span><strong data-summary="produto">R$ 0,00</strong></div>
                            <div><span>Subtotal de outros</span><strong data-summary="outro">R$ 0,00</strong></div>
                            <div><span>Desconto geral</span><strong data-summary="discount">R$ 0,00</strong></div>
                            <div><span>Acréscimo</span><strong data-summary="increase">R$ 0,00</strong></div>
                            <div class="total"><span>Total</span><strong data-summary="total">R$ 0,00</strong></div>
                        </div>
                        <div class="form-group"><label class="form-label">Observações</label><textarea class="form-control-os" name="notes" id="<?= $edit ? 'edit' : 'create' ?>-budget-notes" rows="3"><?= h(budget_value($data, 'notes')) ?></textarea></div>
                    </section>
                </div>
...
<div class="modal fade" id="modal-orcamento-view" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content visual-modal">
            <div class="modal-header">
                <div>
                    <h2 class="modal-title fs-5">Dados do orçamento</h2>
                    <p class="text-muted small mb-0" id="view-budget-subtitle"></p>
                </div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
            </div>
            <div class="modal-body">
                <section class="form-section">
                    <h3 class="form-section-title">Resumo</h3>
                    <div class="summary-box" id="view-budget-summary"></div>
                </section>
                <section class="form-section">
                    ...
                    <div class="table-panel-wrap">
                        <table class="os-table">
                            ...
                            <tbody id="view-budget-items"></tbody>
                        </table>
                    </div>
                </section>
            </div>
...
<div class="modal fade" id="modal-orcamento-status" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content visual-modal" method="post" action="actions/orcamento-status.php">
            <div class="modal-header">
                <h2 class="modal-title fs-5" id="budget-status-title">Alterar status</h2><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
            </div>
            <div class="modal-body"><?= $csrf->field() ?><?php return_to_field(); ?><input type="hidden" name="id" id="status-budget-id"><input type="hidden" name="operation" id="status-budget-operation">
                <p id="budget-status-message"></p>
                <div class="form-group d-none" id="budget-reject-reason-wrap"><label class="form-label">Motivo da recusa</label><textarea class="form-control-os" name="reason" rows="3"></textarea></div>
            </div>
    ...
    <div
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form
                ...
                method="post"
                action="actions/orcamento-excluir.php"
                autocomplete="off">
                <div class="modal-header">
                    <div>
                        <h2
                            class="modal-title fs-5"
                            id="budget-delete-title">
                            Excluir orçamento
                        </h2>
                ...
                <div class="modal-body">
                    ...
                    <div
                        class="alert alert-danger<?= $deleteError === null ? ' d-none' : '' ?>"
                        id="delete-budget-error"
                        role="alert">
                        <?= h($deleteError ?? '') ?>
                    </div>
                    ...
                    <div
                        ...
                        id="delete-budget-warning"
                        role="note">
                        <i class="bi bi-exclamation-triangle"></i>

                        O orçamento
                        <strong id="delete-budget-number">
                            <?= h(
                                budget_value(
                                    $deleteData,
                                    'number',
                        ...
                        será removido das telas operacionais.
                    </div>

                    <div
                        class="alert alert-danger d-none"
                        id="delete-budget-linked-warning"
                        role="alert">
                        <i class="bi bi-shield-lock"></i>

                        Este orçamento possui uma Ordem de Serviço
                        operacional vinculada:

                        <strong id="delete-budget-linked-order"></strong>.

                        Regularize a OS antes de excluir o orçamento.
                    </div>

                    <p class="text-muted small mb-0">
                        Os itens, valores, cliente, usuário responsável
                        e data da exclusão permanecerão preservados para
                        auditoria.
                    </p>
                </div>
...
<template id="budget-item-template">
    <div class="form-row budget-item-row"><input type="hidden" data-field="type">
        ...
        <div class="form-group"><label class="form-label">Subtotal</label><input class="form-control-os" data-field="subtotal" value="R$ 0,00" readonly></div>
        <div class="form-group"><label class="form-label">&nbsp;</label><button class="btn-filter btn-filter-ghost js-remove-budget-item" type="button"><i class="bi bi-trash"></i></button></div>
    </div>
</template>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function setName(row, type, index) {
            row.querySelectorAll('[data-field]').forEach(function(input) {
                input.name = (type === 'servico' ? 'services' : type === 'produto' ? 'products' : 'others') + '[' + index + '][' + input.dataset.field + ']';
            });
        }
        ...
        function recalc(form) {
            ...
            form.querySelectorAll('.budget-item-row').forEach(function(row) {
            ...
            const discount = parseNumber(form.querySelector('.js-budget-discount')?.value);
            const increase = parseNumber(form.querySelector('.js-budget-increase')?.value);
            const total = Math.max(0, sums.servico + sums.produto + sums.outro - discount + increase);
            Object.entries({
                servico: sums.servico,
                produto: sums.produto,
                outro: sums.outro,
                discount,
                increase,
                total
        ...
        function addRow(form, type, item) {
            ...
            row.querySelector('.js-remove-budget-item').addEventListener('click', function() {
                row.remove();
                recalc(form);
            });
        ...
        function restoreRows(form, data) {
            form.querySelectorAll('.budget-items').forEach(function(box) {
                box.replaceChildren();
            });
            (data.services || []).forEach(function(item) {
                addRow(form, 'servico', item);
            });
            (data.products || []).forEach(function(item) {
                addRow(form, 'produto', item);
            });
            (data.others || []).forEach(function(item) {
                addRow(form, 'outro', item);
            });
            recalc(form);
        }
        document.querySelectorAll('.js-budget-form').forEach(function(form) {
            form.querySelectorAll('.js-add-budget-item').forEach(function(button) {
                button.addEventListener('click', function() {
                    addRow(form, button.dataset.type);
                });
            });
            form.querySelectorAll('.js-budget-discount,.js-budget-increase').forEach(function(input) {
                input.addEventListener('input', function() {
                    recalc(form);
                });
            });
            if (!form.closest('#modal-orcamento-edit') && !(recoveryModal === 'create' && hasRows(recoveryData))) addRow(form, 'servico');
        });
        async function loadBudget(id, mode) {
            const response = await fetch('actions/orcamento-detalhes.php?id=' + encodeURIComponent(id) + (mode ? '&mode=' + mode : ''), {
                headers: {
                    Accept: 'application/json'
                }
            });
            if (!response.ok) throw new Error('Falha ao carregar orçamento.');
        ...
        document.addEventListener('click', async function(event) {
            ...
            const button = event.target.closest('.js-budget-view, .js-budget-edit, .js-budget-status, .js-budget-delete');
            if (!button) return;
            if (button.classList.contains('js-budget-view')) {
                const data = await loadBudget(button.dataset.budgetId);
                const budget = data.budget;
                document.getElementById('view-budget-subtitle').textContent = budget.number;
                const summary = document.getElementById('view-budget-summary');
                summary.replaceChildren();
                [
                    ['Número', budget.number],
                    ['Cliente', budget.client_name + ' · ' + (budget.client_document || '-')],
                    ['Emissão', budget.issue_date],
                    ['Validade', budget.valid_until],
                    ['Status', budget.display_status],
                    ['Subtotal serviços', money(parseNumber(budget.services_subtotal))],
                    ['Subtotal produtos', money(parseNumber(budget.products_subtotal))],
                    ['Subtotal outros', money(parseNumber(budget.others_subtotal))],
                    ['Desconto', money(parseNumber(budget.discount))],
                    ['Acréscimo', money(parseNumber(budget.increase))],
                    ['Total', money(parseNumber(budget.total))],
                ...
                const tbody = document.getElementById('view-budget-items');
                tbody.replaceChildren();
                data.items.forEach(function(item) {
                    const row = document.createElement('tr');
                    [item.type, item.description, item.unit, item.quantity, money(parseNumber(item.unit_price)), money(parseNumber(item.discount)), money(parseNumber(item.subtotal))].forEach(function(value) {
            ...
            if (button.classList.contains('js-budget-edit')) {
                const data = await loadBudget(button.dataset.budgetId, 'edit');
                const budget = data.budget;
                const modal = document.getElementById('modal-orcamento-edit');
                ...
                [
                    ...
                    ['issue-date', budget.issue_date],
                    ['valid-until', budget.valid_until],
                    ['status', budget.status],
                    ['notes', budget.notes],
                    ['discount', budget.discount],
                    ['increase', budget.increase]
                ].forEach(function(pair) {
                    const el = document.getElementById('edit-budget-' + pair[0]) || form.querySelector(pair[0] === 'discount' ? '.js-budget-discount' : pair[0] === 'increase' ? '.js-budget-increase' : '');
                    if (el) el.value = pair[1] || '';
                });
                data.items.forEach(function(item) {
                    addRow(form, item.type, item);
                });
                recalc(form);
            }
            if (button.classList.contains('js-budget-status')) {
                const reject = button.dataset.operation === 'reject';
                document.getElementById('status-budget-id').value = button.dataset.budgetId;
                document.getElementById('status-budget-operation').value = button.dataset.operation;
                document.getElementById('budget-status-title').textContent = reject ? 'Recusar orçamento' : 'Aprovar orçamento';
                document.getElementById('budget-status-message').textContent = (reject ? 'Deseja recusar ' : 'Deseja aprovar ') + button.dataset.budgetNumber + '?';
                document.getElementById('budget-reject-reason-wrap').classList.toggle('d-none', !reject);
            }
            if (
                ...
                const budgetNumber =
                    button.dataset.budgetNumber ||
                    'este orçamento';

                const linkedOrder =
                    button.dataset.linkedOrder || '';

                const idInput = document.getElementById(
                    'delete-budget-id'
                );

                const numberNode = document.getElementById(
                    'delete-budget-number'
                );
                ...
                const normalWarning = document.getElementById(
                    'delete-budget-warning'
                );

                const linkedWarning = document.getElementById(
                    'delete-budget-linked-warning'
                );

                const linkedOrderNode =
                    document.getElementById(
                        'delete-budget-linked-order'
                    );

                const submitButton = document.getElementById(
                    'delete-budget-submit'
                );
                ...
                if (numberNode) {
                    numberNode.textContent = budgetNumber;
                }
                ...
                normalWarning?.classList.toggle(
                    'd-none',
                    hasLinkedOrder
                );

                linkedWarning?.classList.toggle(
                    'd-none',
                    !hasLinkedOrder
                );

                if (linkedOrderNode) {
                    linkedOrderNode.textContent =
                        linkedOrder;
                }
        ...
        const targets = {
            create: 'modal-orcamento',
            edit: 'modal-orcamento-edit',
            delete: 'modal-orcamento-delete'
        };
        if (recoveryModal && targets[recoveryModal] && window.bootstrap) {
            const modal = document.getElementById(targets[recoveryModal]);
            if (modal) {
                const form = modal.querySelector('form');
                if (form && hasRows(recoveryData)) restoreRows(form, recoveryData);
                if (
