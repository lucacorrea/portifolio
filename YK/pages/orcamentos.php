...
$filters = [
    ...
    'responsible_id' => trim((string) ($_GET['responsible_id'] ?? '')),
...
<form class="filter-bar" method="get" action="orcamentos.php">
    <div class="search-wrap"><i class="bi bi-search"></i><input class="search-input" type="search" name="search" value="<?= h($filters['search']) ?>" placeholder="Buscar número, cliente ou responsável" maxlength="150"></div>
    ...
    <select class="filter-select" name="responsible_id" aria-label="Responsável"><option value="">Todos os responsáveis</option><?php foreach ($employees as $employee): ?><option value="<?= h((string) $employee->id()) ?>" <?= $filters['responsible_id'] === (string) $employee->id() ? 'selected' : '' ?>><?= h($employee->name()) ?></option><?php endforeach; ?></select>
...
$employeeOptions = array_map(static fn(Employee $employee): array => ['id' => $employee->id(), 'name' => $employee->name()], $employees);
