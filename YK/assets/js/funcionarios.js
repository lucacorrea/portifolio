document.addEventListener('DOMContentLoaded', function () {
  ...
  const pageData = pageDataElement
  ...
  const sections = [
    ...
    [
      ...
      [
        ['endereco', 'Endereço'],
        ['telefone_celular', 'Telefone celular', 'phone'],
    ...
    [
      'Título de eleitor',
      [
        ['titulo_eleitor_numero', 'Número'],
        ['titulo_eleitor_uf', 'UF'],
    ...
    [
      ...
      [
        ['reservista_numero', 'Série'],
        ['reservista_data_emissao', 'Emissão', 'date']
      ]
    ],
    [
      'Certidão de nascimento',
      [
        ['certidao_nascimento_numero', 'Número'],
        ['certidao_nascimento_cidade', 'Cidade'],
        ['certidao_nascimento_livro', 'Livro'],
        ['certidao_nascimento_folha', 'Folha'],
    ...
    [
      ...
      [
        ['carteira_trabalho_numero', 'Número'],
        ['carteira_trabalho_serie', 'Série'],
        ['carteira_trabalho_uf', 'UF'],
    ...
    [
      'Habilitação',
      [
        ['cnh_numero_registro', 'Nº do registro'],
        ['cnh_categoria', 'Categoria'],
        ['cnh_data_vencimento', 'Vencimento', 'date']
      ]
    ],
    [
      'Manequim',
      [
        ['manequim_camisa', 'Camisa'],
        ['manequim_calca', 'Calça'],
        ['manequim_calcado', 'Calçado']
      ]
    ]
  ];
  ...
  function renderView(employee) {
    ...
    const identity = element(
      'section',
      'form-section employee-identity'
    );
    ...
    if (employee.photo_url) {
      ...
      image.alt = 'Foto de ' + employee.name;
    ...
    heading.append(
      element('h3', 'mb-1', employee.name),
      element(
        'p',
        'text-muted mb-0',
        employee.code
          + ' · '
          + formatted(employee.status, 'status')
      )
    );

    identity.append(avatar, heading);
    viewContent.appendChild(identity);

    sections.forEach(function (definition) {
      const available = definition[1].filter(function (field) {
    ...
    [
      ...
      auditGrid.appendChild(item);
    });

    audit.appendChild(auditGrid);
    viewContent.appendChild(audit);
  }

  async function loadEmployee(id, mode) {
    const response = await fetch(
      'actions/funcionario-detalhes.php?id='
  ...
  function populateEdit(employee) {
    ...
    if (subtitle) {
      subtitle.textContent = employee.code;
    }
  ...
  function populateStatusModal(button) {
    const targetStatus =
    ...
    const subtitleNode = document.getElementById(
      'employee-status-subtitle'
    );

    const warningNode = document.getElementById(
    ...
    if (titleNode) {
      titleNode.textContent = isDeactivation
        ? 'Inativar funcionário'
        : 'Reativar funcionário';
    }

    if (subtitleNode) {
      subtitleNode.textContent = isDeactivation
        ? 'O funcionário deixará de aparecer nas novas atribuições.'
        : 'O funcionário voltará a ficar disponível para novas atribuições.';
    }

    if (warningNode) {
      warningNode.className = isDeactivation
        ? 'alert alert-warning mt-3 mb-0'
        : 'alert alert-info mt-3 mb-0';

      warningNode.textContent = isDeactivation
        ? 'A inativação será bloqueada se o funcionário estiver em uma OS operacional ou em um planejamento semanal aguardando confirmação.'
        : 'A reativação não altera o histórico e libera o funcionário para novas Ordens de Serviço e planejamentos.';
    }
    ...
    if (submitButton) {
      ...
      submitButton.innerHTML = isDeactivation
        ? '<i class="bi bi-person-dash"></i> Confirmar inativação'
  ...
  document.addEventListener(
    ...
    async function (event) {
      ...
      if (statusButton) {
        populateStatusModal(statusButton);
        return;
      }
      ...
      const editing = button.classList.contains(
        'js-employee-edit'
      );

      const currentEditSequence = editing
        ? ++editLoadSequence
        : 0;
  ...
  document.querySelectorAll(
    '.js-employee-form, #modal-funcionario-status form'
