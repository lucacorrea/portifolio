...
final class CompanyLogoStorage
{
    ...
    public function store(?array $upload): ?string
    {
        ...
        return 'empresa-logo.php?v=' . $filename;
    }

    public function resolve(string $reference): ?string
    {
        if (!preg_match('#^empresa-logo\.php\?v=([a-f0-9]{32}\.(?:jpg|png|webp))$#', $reference, $matches)) {
            return null;
        }
    ...
    private function inspect(?array $upload): ?array
    {
        ...
        if ($temporaryPath === '' || !is_uploaded_file($temporaryPath)) {
            throw new InvalidArgumentException('O arquivo de logo enviado é inválido.');
        }
        if ($size <= 0 || $size > self::MAX_BYTES) {
            throw new InvalidArgumentException('A logo deve ter no máximo 5 MB.');
        }

        $image = @getimagesize($temporaryPath);
        if (!is_array($image) || !isset($image[0], $image[1], $image['mime'])) {
            throw new InvalidArgumentException('Envie uma logo JPEG, PNG ou WebP válida.');
        }
        ...
        if (!isset(self::MIME_EXTENSIONS[$mime])) {
            throw new InvalidArgumentException('Envie uma logo JPEG, PNG ou WebP.');
        }
        if (
            $width <= 0
            || $height <= 0
            ...
            throw new InvalidArgumentException('A logo possui dimensões acima do limite permitido.');
        }
        ...
        if (function_exists('finfo_open')) {
            ...
            if (!is_string($detected) || strtolower($detected) !== $mime) {
                throw new InvalidArgumentException('O conteúdo da logo não corresponde ao tipo informado.');
            }
        }
    ...
    {
        return match ($error) {
            UPLOAD_ERR_INI_SIZE, UPLOAD_ERR_FORM_SIZE => 'A logo ultrapassa o limite de 5 MB.',
            UPLOAD_ERR_PARTIAL => 'O envio da logo foi interrompido. Tente novamente.',
            default => 'Não foi possível receber a logo enviada.',
        };
    }
}
