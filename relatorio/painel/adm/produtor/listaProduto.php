...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($csrf, $postedCsrf)) {
        ...
        header('Location: ./listaProduto.php?p=' . $returnP);
        exit;
    }
    ...
    try {
        if ($acao === 'toggle') {
            ...
            if ($stmt->rowCount() > 0) {
                $_SESSION['flash_ok'] = 'Status do produto atualizado.';
            } else {
                $_SESSION['flash_err'] = 'Produto não encontrado.';
            }
            header('Location: ./listaProduto.php?p=' . $returnP);
            exit;
        }
        ...
        if ($acao === 'excluir') {
            ...
            if ($stmt->rowCount() > 0) {
                $_SESSION['flash_ok'] = 'Produto excluído com sucesso.';
            } else {
                $_SESSION['flash_err'] = 'Produto não encontrado.';
            }
            header('Location: ./listaProduto.php?p=' . $returnP);
        ...
        if ($mysqlCode === 1451) {
            ...
            $_SESSION['flash_err'] = 'Tabela "produtos" não existe. Rode o SQL das tabelas.';
        } else {
            $sqlState  = (string)$e->getCode();
            $_SESSION['flash_err'] = "Não foi possível concluir a ação agora. (SQLSTATE {$sqlState} / MySQL {$mysqlCode})";
        }
        header('Location: ./listaProduto.php?p=' . $returnP);
        exit;
    } catch (Throwable $e) {
...
try {
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM produtos WHERE feira_id = :feira");
    $stmt->bindValue(':feira', $feiraId, PDO::PARAM_INT);
    $stmt->execute();
                                        ...
                                        <div class="d-flex flex-wrap justify-content-between align-items-center mt-3">
                                            ...
                                            <nav aria-label="Paginação produtos">
                                                <ul class="pagination mb-0">
                                                    ...
                                                    // Janela de páginas (ex.: 1..5)
                                                    $win = 2; // mostra 2 antes e 2 depois
                                                    $ini = max(1, $pagina - $win);
                                                    $fimP = min($totalPaginas, $pagina + $win);
                                                    ?>

                                                    <li class="page-item <?= $disabledPrev ?>">
                                                        <a class="page-link" href="./listaProduto.php?p=<?= (int)$prev ?>" tabindex="-1">Anterior</a>
                                                    </li>

                                                    <?php if ($ini > 1): ?>
                                                        <li class="page-item"><a class="page-link" href="./listaProduto.php?p=1">1</a></li>
                                                        <?php if ($ini > 2): ?>
                                                            <li class="page-item disabled"><span class="page-link">…</span></li>
                                                        <?php endif; ?>
                                                    <?php endif; ?>

                                                    <?php for ($i = $ini; $i <= $fimP; $i++): ?>
                                                        <li class="page-item <?= ($i === $pagina) ? 'active' : '' ?>">
                                                            <a class="page-link" href="./listaProduto.php?p=<?= (int)$i ?>"><?= (int)$i ?></a>
                                                        </li>
                                                    <?php endfor; ?>

                                                    <?php if ($fimP < $totalPaginas): ?>
                                                        <?php if ($fimP < $totalPaginas - 1): ?>
                                                            <li class="page-item disabled"><span class="page-link">…</span></li>
                                                        <?php endif; ?>
                                                        <li class="page-item"><a class="page-link" href="./listaProduto.php?p=<?= (int)$totalPaginas ?>"><?= (int)$totalPaginas ?></a></li>
                                                    <?php endif; ?>

                                                    <li class="page-item <?= $disabledNext ?>">
                                                        <a class="page-link" href="./listaProduto.php?p=<?= (int)$next ?>">Próximo</a>
                                                    </li>
