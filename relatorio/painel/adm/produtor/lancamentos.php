...
/**
 * SEGURANÇA: segura qualquer saída indevida (BOM/espaço/warning de include)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // força salvar somente no dia que a tela está
  if ($dataRef !== $dia) {
    header('Location: ./lancamentos.php?dia=' . urlencode($dia));
    exit;
  }
...
/**
 * Limpa qualquer output “segurado” (BOM/notice) antes do HTML,
 * sem perder o funcionamento normal.
 */
...
<head>
  ...
  <style>
    ...
    .card-header-lite{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;border-bottom:1px solid rgba(0,0,0,.06);padding-bottom:12px;margin-bottom:12px}
    .pill{display:inline-flex;align-items:center;gap:6px;padding:6px 10px;border-radius:999px;font-size:12px;font-weight:700;background:#eef2ff;color:#1f2a6b}
    .totbox{border:1px solid rgba(0,0,0,.08);background:#fff;border-radius:12px;padding:10px 12px;min-width:170px}
    .totlabel{font-size:12px;color:#6c757d;margin:0}
    .totvalue{font-size:20px;font-weight:900;margin:0}
    .line-card{border:1px solid rgba(0,0,0,.08);background:#fff;border-radius:14px;padding:12px;margin-bottom:10px}
    .mini{height:38px!important}
    .muted{color:#6c757d}
    .photo-thumb{width:76px;height:52px;object-fit:cover;border-radius:10px;border:1px solid rgba(0,0,0,.12);display:none}
    .sticky-actions{position:sticky;bottom:10px;z-index:3;background:rgba(255,255,255,.92);border:1px solid rgba(0,0,0,.08);border-radius:14px;padding:10px;backdrop-filter:blur(6px);display:flex;flex-wrap:wrap;gap:10px;justify-content:space-between;align-items:center;margin-top:12px}

    /* Toast */
    .sig-flash-wrap{position:fixed;top:78px;right:18px;width:min(420px,calc(100vw - 36px));z-index:9999;pointer-events:none}
    .sig-toast.alert{pointer-events:auto;border:0!important;border-left:6px solid!important;border-radius:14px!important;padding:10px 12px!important;box-shadow:0 10px 28px rgba(0,0,0,.10)!important;font-size:13px!important;margin-bottom:10px!important;opacity:0;transform:translateX(10px);animation:sigToastIn .22s ease-out forwards,sigToastOut .25s ease-in forwards 5.75s}
      ...
      <ul class="nav">
        ...
        <li class="nav-item">
          ...
          <div class="collapse" id="feiraMovimento">
            ...
            <ul class="nav flex-column sub-menu" style="background:#fff !important;">
              <li class="nav-item">
                <a class="nav-link active" href="./lancamentos.php?dia=<?= h($dia) ?>" style="color:white !important; background: #231475C5 !important;">
