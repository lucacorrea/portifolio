...
$perfisSess = $_SESSION['perfis'] ?? [];
if (!is_array($perfisSess)) $perfisSess = [$perfisSess];

if (!in_array('ADMIN', $perfisSess, true)) {
...
/* =========================
   CARREGA PERFIS DO BANCO (tabela perfis)
   - select usa "codigo" (ADMIN/OPERADOR)
   ========================= */
$perfisDb = [];
try {
  $st = $pdo->query("SELECT id, codigo, nome FROM perfis ORDER BY nome ASC");
  $perfisDb = $st->fetchAll(PDO::FETCH_ASSOC) ?: [];
} catch (Throwable $e) {
  $perfisDb = [];
}

/* fallback se não existir perfis cadastrados */
if (empty($perfisDb)) {
  $perfisDb = [
    ['id' => 0, 'codigo' => 'OPERADOR', 'nome' => 'OPERADOR'],
    ['id' => 0, 'codigo' => 'ADMIN', 'nome' => 'ADMIN'],
  ];
}
...
try {
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($csrf, $token)) {
      ...
      if ($nome === '' || mb_strlen($nome) < 3) {
      ...
      } elseif (!in_array($perfil, array_map(fn($p) => strtoupper((string)$p['codigo']), $perfisDb), true)) {
        $err = "Perfil inválido.";
        ...
        if ($existe) {
          ...
          // pega o perfil_id pelo codigo (tabela perfis)
          $st = $pdo->prepare("SELECT id FROM perfis WHERE UPPER(codigo) = :cod LIMIT 1");
          $st->execute([':cod' => $perfil]);
          $perfilId = (int)($st->fetchColumn() ?? 0);

          if ($perfilId <= 0) {
            $err = "Perfil não encontrado no banco (tabela perfis). Cadastre o perfil primeiro.";
          } else {
            ...
            // vincula perfil (tabela usuario_perfis)
            $st = $pdo->prepare("
                    ...
                    <div class="form-row">
                      <div class="form-group col-md-6">
                        ...
                        <select id="perfil" class="form-control" name="perfil">
                          <?php foreach ($perfisDb as $p): ?>
                            <?php
                              $cod = strtoupper((string)($p['codigo'] ?? ''));
                              $nm  = (string)($p['nome'] ?? $cod);
                            ?>
                            <option value="<?= h($cod) ?>" <?= $perfil === $cod ? 'selected' : '' ?>>
                              <?= h($nm) ?>
                            </option>
                          <?php endforeach; ?>
                        </select>
                        <div class="hint">Os perfis vêm da tabela <b>perfis</b> (campo <b>codigo</b>).</div>
                      </div>
                      <div class="form-group col-md-6">
