...
function render_aquisicao_lote_signature_footer(string $assinaturaLabel, bool $isSupport): void
{
?>
...
function render_aquisicao_lote_page(array $aq, array $items, string $via, bool $isSupport, int $pagina): void
{
    ...
    <div class="card printable-page" data-page="<?= $pagina ?>" data-aquisicao="<?= (int)$aq['id'] ?>">
        ...
        <div class="card-body">
            <table class="print-repeat-table">
                        ...
                        <td>
                            <div class="ordem-info-wrap">
                                <table class="ordem-info-table">
                                    <tr>
                                        <td class="ordem-info-label label-fornecedor">Fornecedor:</td>
                                        <td class="strong"><?= h_lote(strtoupper((string)$aq['fornecedor'])) ?></td>
                                        <td class="ordem-info-label label-emissao">Local e Data de Emissão:</td>
                ...
                <tfoot>
                    <tr><td><?php render_aquisicao_lote_signature_footer($assinatura_label, $isSupport); ?></td></tr>
                </tfoot>
            </table>
        </div>
    </div>
...
<style>
    ...
    /* Cabeçalho simétrico: logo e bloco da ordem ocupam a mesma largura. */
    .ordem-header{display:grid;grid-template-columns:220px minmax(0,1fr) 220px;align-items:center;border-bottom:2px solid #000;padding:0 0 1rem;margin-bottom:1.35rem;gap:1rem;min-height:95px}
    .ordem-logo{display:flex;align-items:center;justify-content:flex-start;min-width:0}.ordem-logo img{display:block;max-height:72px;max-width:180px;width:auto;height:auto;object-fit:contain}
    .ordem-center{text-align:center;min-width:0;padding:0 .5rem}.ordem-center h1{font-size:1.28rem;line-height:1.15;font-weight:900;margin:0;color:#000;text-transform:uppercase;white-space:nowrap}.ordem-center h2{font-size:.82rem;line-height:1.25;font-weight:800;margin:4px 0 0;color:#222;text-transform:uppercase}.ordem-cnpj{font-size:.7rem;line-height:1.25;margin-top:5px;color:#555;font-weight:600}
    .ordem-right{display:flex;flex-direction:column;align-items:stretch;justify-content:center;text-align:center;width:100%;min-width:0}.via-label{font-weight:800;color:#777;font-size:.65rem;text-transform:uppercase;margin-bottom:5px;letter-spacing:.08em;text-align:center}.ordem-right-box{border:1.5px solid #000;padding:.35rem .55rem;width:100%;box-sizing:border-box;text-align:center}.ordem-label{font-size:.6rem;font-weight:800;color:#000;text-transform:uppercase}.ordem-numero{font-size:1.28rem;font-weight:900;color:#000;line-height:1.05;white-space:nowrap}.ordem-data{font-size:.67rem;color:#444;margin-top:5px;font-weight:700;text-transform:uppercase;text-align:center;white-space:nowrap}

    .ordem-info-table,.ordem-items-table,.print-repeat-table{width:100%;border-collapse:collapse}.print-repeat-table>thead>tr>td,.print-repeat-table>tfoot>tr>td,.print-repeat-table>tbody>tr>td{padding:0;border:0}.ordem-info-wrap,.ordem-items-wrap{width:100%;overflow-x:auto;-webkit-overflow-scrolling:touch}.ordem-info-wrap{margin-bottom:1rem}.ordem-info-table{margin-bottom:0;font-size:.8125rem;table-layout:fixed}.ordem-items-table{font-size:.8125rem;border:1px solid #000;table-layout:fixed}
    .ordem-info-table td,.ordem-items-table th,.ordem-items-table td{border:1px solid #000;padding:7px 8px;vertical-align:middle;line-height:1.3}.ordem-items-table thead tr,.ordem-total-row,.ordem-info-label{background:#f2f2f2}.ordem-items-table th{text-align:center;font-size:.72rem;font-weight:800;text-transform:uppercase}.ordem-items-table td{font-size:.8rem}.ordem-info-label{font-weight:800;font-size:.69rem;text-transform:uppercase;white-space:nowrap}
    .label-fornecedor{width:14%}.label-emissao{width:25%}.data-emissao{width:20%}.referencia{font-family:monospace;font-weight:900;letter-spacing:.5px;overflow-wrap:anywhere}.c-item{width:7%}.c-unid{width:8%}.c-qtd{width:9%}.c-preco{width:15%}.c-total{width:16%}.valor-monetario{white-space:nowrap;font-variant-numeric:tabular-nums}.ordem-total-row td{border-bottom:1px solid #000!important}.total-label{font-size:.84rem;text-transform:uppercase;padding-right:10px!important}.total-value{font-size:.92rem;white-space:nowrap;font-variant-numeric:tabular-nums}.ordem-section-title{font-size:.76rem;font-weight:800;color:#222;text-transform:uppercase;margin:1.15rem 0 .45rem}

    /* Área de assinatura com linha fixa e imagem centralizada sem deslocar o texto. */
    .rodape-documento{margin-top:1.35rem}.assinaturas-grid{display:grid;grid-template-columns:1fr 1fr;gap:3rem;text-align:center;margin-top:2rem}.assinatura-linha{position:relative;min-height:92px;padding:58px 8px 0;box-sizing:border-box}.assinatura-linha::before{content:"";position:absolute;left:3%;right:3%;top:55px;border-top:1.5px solid #000}.assinatura-titulo{font-weight:800;color:#000;font-size:.82rem;line-height:1.2;margin-top:7px}.assinatura-subtitulo{font-size:.63rem;color:#555;font-weight:700;text-transform:uppercase;margin-top:3px}.support-signature-image{display:none;position:absolute;left:50%;top:0;transform:translateX(-50%);width:auto;max-width:76%;height:auto;max-height:55px;object-fit:contain;z-index:2}.printable-page.signature-enabled .support-signature-image{display:block}

    @media(max-width:768px){.print-topbar{flex-direction:column;align-items:stretch}.print-topbar .btn,.print-options .btn,.print-options .form-group,.support-signature-panel,.support-signature-panel .form-group{width:100%;justify-content:center;text-align:center}.print-options{width:100%}.printable-page .card-body{padding:1rem}.ordem-header{grid-template-columns:1fr;text-align:center}.ordem-right,.ordem-logo{justify-self:center;text-align:center;align-items:center;justify-content:center;width:100%;max-width:260px;margin:0 auto}.ordem-center h1{white-space:normal}.assinaturas-grid{grid-template-columns:1fr;gap:2rem}.ordem-info-table,.ordem-items-table{min-width:760px}.signature-page-control{position:static;margin:10px}}
    ...
    @media print{
        ...
        .page-body,.container-xl,.print-doc{width:100%!important;max-width:100%!important;margin:0!important;padding:0!important}
        .printable-page{display:block!important;width:100%!important;max-width:100%!important;margin:0!important;padding:0!important;border:0!important;box-shadow:none!important;background:#fff!important;page-break-after:always;break-after:page;border-radius:0!important;overflow:visible!important}
        .print-doc .printable-page:last-child{page-break-after:auto;break-after:auto}.printable-page .card-body{padding:0!important;margin:0!important}

        .ordem-header{display:grid!important;grid-template-columns:42mm minmax(0,1fr) 42mm!important;align-items:center!important;gap:4mm!important;min-height:24mm!important;margin:0 0 4mm!important;padding:0 0 3mm!important;border-bottom:1.5px solid #000!important}
        .ordem-logo{display:flex!important;align-items:center!important;justify-content:flex-start!important;margin:0!important;padding:0!important;width:42mm!important}.ordem-logo img{display:block!important;width:auto!important;height:auto!important;max-width:40mm!important;max-height:18mm!important;margin:0!important}
        .ordem-center{padding:0!important;margin:0!important;text-align:center!important;min-width:0!important}.ordem-center h1{font-size:15px!important;line-height:1.1!important;white-space:nowrap!important;margin:0!important}.ordem-center h2{font-size:9px!important;line-height:1.2!important;margin:2mm 0 0!important}.ordem-cnpj{font-size:7.5px!important;line-height:1.2!important;margin-top:1mm!important}
        .ordem-right{display:flex!important;flex-direction:column!important;align-items:stretch!important;justify-content:center!important;width:42mm!important;margin:0!important;padding:0!important;text-align:center!important}.via-label{font-size:7px!important;margin:0 0 1mm!important;text-align:center!important}.ordem-right-box{width:100%!important;box-sizing:border-box!important;padding:1.2mm 1mm!important;border:1.2px solid #000!important}.ordem-label{font-size:6.5px!important}.ordem-numero{font-size:15px!important;line-height:1!important}.ordem-data{font-size:6.8px!important;line-height:1.1!important;margin-top:1.2mm!important;text-align:center!important;white-space:nowrap!important}
        ...
        .ordem-info-table{font-size:9px!important}.ordem-items-table{font-size:9px!important;border:1px solid #000!important}.ordem-items-table thead{display:table-header-group!important}.ordem-info-table td,.ordem-items-table th,.ordem-items-table td{padding:2.2mm 1.8mm!important;border:1px solid #000!important;vertical-align:middle!important;line-height:1.22!important}.ordem-info-label{font-size:7.5px!important}.ordem-items-table th{font-size:7.5px!important;line-height:1.15!important}.ordem-items-table td{font-size:9px!important}.label-fornecedor{width:14%!important}.label-emissao{width:25%!important}.data-emissao{width:20%!important}.c-item{width:7%!important}.c-unid{width:8%!important}.c-qtd{width:9%!important}.c-preco{width:15%!important}.c-total{width:16%!important}.valor-monetario,.total-value{white-space:nowrap!important;font-variant-numeric:tabular-nums!important}.ordem-section-title{margin:3.5mm 0 1.5mm!important;font-size:8.5px!important;line-height:1.2!important}
        .ordem-total-row,.ordem-total-row td,.rodape-documento{page-break-inside:avoid!important;break-inside:avoid!important}.total-label{font-size:9px!important}.total-value{font-size:10px!important}

        .rodape-documento{margin-top:5mm!important}.assinaturas-grid{display:grid!important;grid-template-columns:1fr 1fr!important;gap:16mm!important;text-align:center!important;margin-top:6mm!important;page-break-inside:avoid!important;break-inside:avoid!important}.assinatura-linha{position:relative!important;min-height:27mm!important;padding:17mm 2mm 0!important;box-sizing:border-box!important}.assinatura-linha::before{content:""!important;position:absolute!important;left:3%!important;right:3%!important;top:16mm!important;border-top:1px solid #000!important}.assinatura-titulo{font-size:9px!important;line-height:1.15!important;margin-top:1.5mm!important}.assinatura-subtitulo{font-size:7px!important;line-height:1.15!important;margin-top:.8mm!important}.support-signature-image{position:absolute!important;left:50%!important;top:0!important;transform:translateX(-50%)!important;width:auto!important;max-width:58mm!important;height:auto!important;max-height:15mm!important;margin:0!important;object-fit:contain!important;z-index:2!important}
    }
</style>
...
<?php else: ?>
    <div class="print-doc" id="print-doc">
        ...
        <?php foreach ($aquisicoes as $aq): ?>
            <?php
            $aq_id = (int)$aq['id'];
            render_aquisicao_lote_page($aq, $itens_por_aquisicao[$aq_id] ?? [], $via, $is_support, $pagina++);
            ?>
        <?php endforeach; ?>
    </div>
<?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const pages = Array.from(document.querySelectorAll('.printable-page'));
