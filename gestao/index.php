...
function dashboardPaymentMethodLabel(string $method): string
{
    ...
    return [
        ...
        'conta_cliente' => 'Conta cliente / Fiado',
        'misto' => 'Misto',
        'outro' => 'Outro',
    ...
    <section class="dashboard-section">
      ...
      <div class="dashboard-kpi-grid">
        <article class="dashboard-kpi-card success">
          <span class="dashboard-kpi-label">Vendas hoje</span>
          <strong class="dashboard-kpi-value"><?= e(dashboardMoney($totalVendidoHoje)) ?></strong>
          <small class="dashboard-kpi-meta">
            <?= (int)$qtdVendasHoje ?> venda<?= $qtdVendasHoje === 1 ? '' : 's' ?> · Ticket <?= e(dashboardMoney($ticketMedioHoje)) ?>
          </small>
        </article>

        <article class="dashboard-kpi-card">
          <span class="dashboard-kpi-label">Lucro estimado hoje</span>
          <strong class="dashboard-kpi-value"><?= e(dashboardMoney($lucroHoje)) ?></strong>
          <small class="dashboard-kpi-meta">
            <?= e((string)dashboardSafePercent($lucroHoje, $totalVendidoHoje)) ?>% sobre o total vendido
          </small>
        </article>

        <article class="dashboard-kpi-card">
          <span class="dashboard-kpi-label">Vendas do mês</span>
          <strong class="dashboard-kpi-value"><?= e(dashboardMoney($totalVendidoMes)) ?></strong>
          <small class="dashboard-kpi-meta">
            <?= (int)$qtdVendasMes ?> venda<?= $qtdVendasMes === 1 ? '' : 's' ?> · Lucro <?= e(dashboardMoney($lucroMes)) ?>
          </small>
        </article>

        <article class="dashboard-kpi-card <?= $totalContasVencidas > 0 ? 'danger' : 'warning' ?>">
          <span class="dashboard-kpi-label">Fiado em aberto</span>
          <strong class="dashboard-kpi-value"><?= e(dashboardMoney($totalContasAbertas)) ?></strong>
          <small class="dashboard-kpi-meta">
            <?= (int)$qtdContasAbertas ?> conta<?= $qtdContasAbertas === 1 ? '' : 's' ?> ·
            <?= (int)$qtdClientesComDivida ?> cliente<?= $qtdClientesComDivida === 1 ? '' : 's' ?>
          </small>
        </article>
      </div>
    </section>
      ...
      <div class="dashboard-card">
        ...
        <div class="dashboard-card-body">
          <div class="dashboard-mini-chart" aria-label="Evolução de vendas dos últimos dias">
            <?php foreach ($chartDays as $row): ?>
              <?php
                $date = (string)($row['sale_date'] ?? '');
                $value = (float)($row['total_sales'] ?? 0);
                $height = $maxEvolutionValue > 0 ? max(5, (int)round(($value / $maxEvolutionValue) * 112)) : 5;
              ?>
      ...
      <div class="dashboard-card">
        ...
        <div class="dashboard-card-body">
          <div class="dashboard-finance-list">
            <div class="dashboard-finance-item">
            ...
            <div class="dashboard-finance-item">
              ...
              <strong class="<?= $totalContasVencidas > 0 ? 'danger' : 'success' ?>">
            ...
            <div class="dashboard-finance-item">
              ...
              <strong><?= (int)$qtdClientesComDivida ?></strong>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="dashboard-data-list">
          ...
          <?php else: ?>
            <?php foreach ($latestSales as $sale): ?>
              <?php $saleStatus = (string)($sale['status'] ?? ''); ?>
              <div class="dashboard-data-row clickable" onclick="location.href='pages/venda-detalhes.php?id=<?= (int)$sale['id'] ?>'">
                <div class="dashboard-row-icon">
                ...
                <div class="dashboard-row-main">
                  ...
                  <span class="dashboard-status <?= e(dashboardSaleStatusClass($saleStatus)) ?>">
                    <?= e(dashboardSaleStatusLabel($saleStatus)) ?>
                  </span>
                </div>

                <div class="dashboard-row-value">
                  <strong><?= e(dashboardMoney($sale['total'] ?? 0)) ?></strong>
                </div>
              </div>
            <?php endforeach; ?>
          <?php endif; ?>
      ...
      <div class="dashboard-card">
        ...
        <div class="dashboard-card-body">
          <?php if (empty($paymentMethods)): ?>
            <p class="dashboard-empty">Nenhum pagamento registrado hoje.</p>
          <?php else: ?>
            <div class="dashboard-payment-list">
              <?php foreach ($paymentMethods as $payment): ?>
                <?php
                  $methodValue = (float)($payment['total_value'] ?? 0);
                  $percent = dashboardSafePercent($methodValue, $totalPaymentMethods);
                ?>
                <article class="dashboard-payment-item">
                  <div class="dashboard-payment-top">
                    <strong><?= e(dashboardPaymentMethodLabel((string)($payment['metodo'] ?? ''))) ?></strong>
                    <span><?= (int)($payment['total_count'] ?? 0) ?> lançamento<?= (int)($payment['total_count'] ?? 0) === 1 ? '' : 's' ?></span>
                  </div>
                  ...
                  <div class="dashboard-payment-bottom">
                    <span><?= e((string)$percent) ?>% do total</span>
                    <b><?= e(dashboardMoney($methodValue)) ?></b>
                  </div>
                </article>
              <?php endforeach; ?>
            </div>
        ...
        <div class="dashboard-data-list">
          ...
          <?php else: ?>
            <?php foreach ($lowStock as $product): ?>
              <div class="dashboard-data-row clickable" onclick="location.href='pages/produto-form.php?id=<?= (int)$product['id'] ?>'">
                <div class="dashboard-row-icon warning">
                  <svg viewBox="0 0 24 24"><path d="M5 7h14v12H5z"/><path d="M8 7a4 4 0 0 1 8 0"/></svg>
                </div>
                ...
                <div class="dashboard-row-value">
                  <strong><?= e(dashboardNumber($product['quantidade'] ?? 0)) ?> un.</strong>
                </div>
              </div>
            <?php endforeach; ?>
          <?php endif; ?>
        ...
        <div class="dashboard-data-list">
          ...
          <?php else: ?>
            <?php foreach ($expiredProducts as $product): ?>
              <div class="dashboard-data-row clickable" onclick="location.href='pages/produto-form.php?id=<?= (int)$product['id'] ?>'">
                <div class="dashboard-row-icon danger">
                ...
                <div class="dashboard-row-main">
                  ...
                  <span class="dashboard-status danger">Vencido</span>
                </div>
            ...
            <?php foreach ($expiringProducts as $product): ?>
              <div class="dashboard-data-row clickable" onclick="location.href='pages/produto-form.php?id=<?= (int)$product['id'] ?>'">
                ...
                <div class="dashboard-row-main">
                  ...
                  <span class="dashboard-status warning">Próximo vencimento</span>
                </div>
        ...
        <div class="dashboard-data-list">
          ...
          <?php else: ?>
            <?php foreach ($topProducts as $product): ?>
              <div class="dashboard-data-row clickable" onclick="location.href='pages/produto-form.php?id=<?= (int)$product['id'] ?>'">
                <div class="dashboard-row-icon success">
                  <svg viewBox="0 0 24 24"><path d="M5 7h14v12H5z"/><path d="M8 7a4 4 0 0 1 8 0"/></svg>
                </div>
                ...
                <div class="dashboard-row-value">
                  <strong><?= e(dashboardNumber($product['total_vendido'] ?? 0)) ?> un.</strong>
                </div>
              </div>
            <?php endforeach; ?>
          <?php endif; ?>
