...
function money_to_number(string $txt): float {
  $s = trim($txt);
  if ($s === '') return 0.0;
  $s = preg_replace('~[^\d,.\-]~', '', $s);
...
function number_to_br(float $v): string {
...
/** Próximo pedido: PDV000001 */
function next_pedido(PDO $pdo): string {
  $sql = "SELECT IFNULL(MAX(CAST(SUBSTRING(pedido,4) AS UNSIGNED)),0) AS mx
          FROM saidas
          WHERE pedido REGEXP '^PDV[0-9]+$'";
  $mx = (int)$pdo->query($sql)->fetchColumn();
  $n  = $mx + 1;
  return 'PDV' . str_pad((string)$n, 6, '0', STR_PAD_LEFT);
}

/** Lista “Últimos cupons” agrupando por pedido */
function last_sales(PDO $pdo, int $limit = 10): array {
  $stmt = $pdo->prepare("
    SELECT
      s.pedido,
      s.data,
      ...
      MAX(s.id) AS last_id
    FROM saidas s
    GROUP BY s.pedido, s.data, s.cliente, s.canal, s.pagamento
    ORDER BY s.data DESC, last_id DESC
    LIMIT {$limit}
  ");
  $stmt->execute();
...
if ($action !== '') {
  ...
  // SEARCH: /vendas.php?action=search&q=...
  if ($action === 'search') {
    ...
    $stmt = $pdo->prepare("
      ...
      FROM produtos
      WHERE status = 'ATIVO'
        AND (nome LIKE :q OR codigo LIKE :q)
      ORDER BY
        (codigo = :exact) DESC,
        nome ASC
      LIMIT 30
  ...
  // LAST: /vendas.php?action=last
  if ($action === 'last') {
    json_out([
      'ok' => true,
      'items' => last_sales($pdo, 10),
      'next_pedido' => next_pedido($pdo),
    ]);
  }

  // CONFIRM: POST JSON /vendas.php?action=confirm
  if ($action === 'confirm') {
    if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
      json_out(['ok' => false, 'msg' => 'Requisição inválida.'], 405);
    }
    ...
    // CSRF (compatível com fetch JSON)
    $posted = (string)($payload['csrf_token'] ?? '');
    $sess   = (string)($_SESSION['csrf_token'] ?? '');
    if (!$posted || !$sess || !hash_equals($sess, $posted)) {
      json_out(['ok' => false, 'msg' => 'CSRF inválido. Recarregue a página.'], 419);
    }

    $itemsReq = $payload['items'] ?? null;
    if (!is_array($itemsReq) || count($itemsReq) < 1) {
      json_out(['ok' => false, 'msg' => 'Adicione pelo menos 1 item.'], 422);
    }
    ...
    $feeTxt  = (string)($delivery['fee'] ?? '0,00');
    $fee     = ($deliveryMode === 'DELIVERY') ? money_to_number($feeTxt) : 0.0;
    $obs     = trim((string)($delivery['obs'] ?? ''));

    if ($deliveryMode === 'DELIVERY' && $address === '') {
      json_out(['ok' => false, 'msg' => 'Informe o endereço do Delivery.'], 422);
    }

    $discount = $payload['discount'] ?? [];
    $dTipo  = strtoupper(trim((string)($discount['tipo'] ?? 'PERC')));
    $dValor = (string)($discount['valor'] ?? '0');
    if (!in_array($dTipo, ['PERC', 'VALOR'], true)) $dTipo = 'PERC';

    $pay = $payload['pay'] ?? [];
    $payMode = strtoupper(trim((string)($pay['mode'] ?? 'UNICO'))); // UNICO|MULTI
    if (!in_array($payMode, ['UNICO', 'MULTI'], true)) $payMode = 'UNICO';

    $paySelected = strtoupper(trim((string)($pay['selected'] ?? 'DINHEIRO'))); // DINHEIRO|PIX|CARTAO|BOLETO
    if (!in_array($paySelected, ['DINHEIRO','PIX','CARTAO','BOLETO'], true)) $paySelected = 'DINHEIRO';

    $paidTxt = (string)($pay['paid'] ?? '0,00');
    $paidVal = money_to_number($paidTxt);

    $payParts = $pay['parts'] ?? [];
    if (!is_array($payParts)) $payParts = [];

    // Carrega produtos do banco (preço/estoque “de verdade”)
    $selProd = $pdo->prepare("SELECT id, codigo, nome, unidade, preco, estoque, status, imagem FROM produtos WHERE id = ? LIMIT 1");
    $updStock = $pdo->prepare("UPDATE produtos SET estoque = estoque - ? WHERE id = ? AND estoque >= ?");
    $insSaida = $pdo->prepare("
      INSERT INTO saidas (data, pedido, cliente, canal, pagamento, produto_id, unidade, qtd, preco, total, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
    ");
    ...
    foreach ($itemsReq as $it) {
      ...
      // PDV aqui é “por unidade” (inteiro). Se quiser decimal depois, dá pra liberar.
      $qtyInt = (int)round($qty);
      $qtyInt = max(1, $qtyInt);

      $selProd->execute([$pid]);
      $p = $selProd->fetch(PDO::FETCH_ASSOC);
      if (!$p) json_out(['ok' => false, 'msg' => "Produto inválido (ID {$pid})."], 422);
      if (strtoupper((string)$p['status']) !== 'ATIVO') json_out(['ok' => false, 'msg' => "Produto inativo: {$p['nome']}."], 422);

      $stock = (int)$p['estoque'];
      if ($stock < $qtyInt) {
        json_out(['ok' => false, 'msg' => "Estoque insuficiente: {$p['nome']} (Disp.: {$stock})."], 422);
      }
      ...
      $cart[] = [
        ...
        'name' => (string)$p['nome'],
        'unit' => (string)($p['unidade'] ?? ''),
        'price_base' => (float)$p['preco'],
        'qty' => $qtyInt,
        'img' => img_url_from_db((string)($p['imagem'] ?? '')),
      ];
    }

    if (count($cart) < 1) {
      json_out(['ok' => false, 'msg' => 'Adicione pelo menos 1 item válido.'], 422);
    }

    // Totais
    $sub = 0.0;
    foreach ($cart as $it) {
      $sub += ($it['qty'] * $it['price_base']);
    }

    $desc = 0.0;
    $dv = money_to_number($dValor);
    if ($dv > 0) {
      if ($dTipo === 'PERC') {
        $dv = min(100.0, $dv);
        $desc = ($sub * $dv) / 100.0;
    ...
    $baseAfterDiscount = max(0.0, $sub - $desc);
    $total = max(0.0, $baseAfterDiscount + $fee);

    if ($total <= 0.0) {
      json_out(['ok' => false, 'msg' => 'Total inválido.'], 422);
    }

    // Validação pagamento (igual ao front)
    $troco = 0.0;
    $payDB = 'DINHEIRO';
    $payNote = '';
    ...
    if ($payMode === 'UNICO') {
      ...
      if ($paySelected === 'DINHEIRO') {
        if ($paidVal + 0.0009 < $total) {
          json_out(['ok' => false, 'msg' => 'No dinheiro, o valor pago deve ser >= total.'], 422);
        }
        $troco = max(0.0, $paidVal - $total);
      } else {
        if (abs($paidVal - $total) > 0.009) {
          json_out(['ok' => false, 'msg' => 'Para Pix/Cartão/Boleto, o valor pago deve ser igual ao total.'], 422);
        }
      }

      $payNote = "PAGO: R$ " . number_to_br($paidVal) . ($paySelected === 'DINHEIRO' ? " TROCO: R$ " . number_to_br($troco) : '');
    } else {
      // MULTI
      $rows = [];
      foreach ($payParts as $r) {
        if (!is_array($r)) continue;
        $m = strtoupper(trim((string)($r['method'] ?? 'PIX')));
        if (!in_array($m, ['DINHEIRO','PIX','CARTAO','BOLETO'], true)) $m = 'PIX';
        $v = money_to_number((string)($r['value'] ?? '0'));
        if ($v <= 0) continue;
      ...
      if (abs($diff) < 0.009) $ok = true;
      else if ($diff > 0.009 && $hasCash) { $ok = true; $troco = $diff; }

      if (!$ok) {
        json_out(['ok' => false, 'msg' => 'Pagamento múltiplo inválido. Ajuste os valores.'], 422);
      }

      $payDB = 'MULTI';
      $payNoteParts = [];
      foreach ($rows as $r) {
        $payNoteParts[] = $r['method'] . ' ' . number_to_br((float)$r['value']);
      }
      $payNote = 'PAG: ' . implode('; ', $payNoteParts) . ($troco > 0 ? ' | TROCO: ' . number_to_br($troco) : '');
    }

    // Monta cliente (com observações compactas) — cabe em 180
    $noteParts = [];
    if ($deliveryMode === 'DELIVERY') {
      $noteParts[] = 'END: ' . str_limit($address, 90);
      if ($fee > 0) $noteParts[] = 'TX: ' . number_to_br($fee);
      if ($obs !== '') $noteParts[] = 'OBS: ' . str_limit($obs, 60);
    }
    if ($desc > 0) $noteParts[] = 'DESC: ' . number_to_br($desc);
    if ($payNote !== '') $noteParts[] = $payNote;

    $clienteDB = $customer;
    if (!empty($noteParts)) $clienteDB .= ' | ' . implode(' | ', $noteParts);
    $clienteDB = str_limit($clienteDB, 180);

    // Alocação do desconto nos itens (para o somatório bater)
    $factor = ($sub > 0.0) ? ($baseAfterDiscount / $sub) : 1.0;

    $itemsDB = [];
    $sumLines = 0.0;
    for ($i = 0; $i < count($cart); $i++) {
      $it = $cart[$i];
      $unitEff = round($it['price_base'] * $factor, 2);
      $lineTot = round($unitEff * $it['qty'], 2);
      $sumLines += $lineTot;
    ...
    // Ajuste de arredondamento (somatório = baseAfterDiscount)
    $diffRound = round($baseAfterDiscount - $sumLines, 2);
    if (abs($diffRound) >= 0.01 && count($itemsDB) > 0) {
      $itemsDB[count($itemsDB) - 1]['line_total'] = round($itemsDB[count($itemsDB) - 1]['line_total'] + $diffRound, 2);
    }

    // Taxa entrega entra no total do último item (pra somar certo no GROUP BY)
    if ($fee > 0 && count($itemsDB) > 0) {
      $itemsDB[count($itemsDB) - 1]['line_total'] = round($itemsDB[count($itemsDB) - 1]['line_total'] + $fee, 2);
    }
    ...
    try {
      ...
      $pedido = next_pedido($pdo);

      // baixa estoque + insere linhas
      foreach ($itemsDB as $it) {
        // baixa estoque (garante >=)
        $updStock->execute([$it['qty'], $it['id'], $it['qty']]);
        if ($updStock->rowCount() !== 1) {
          $pdo->rollBack();
          json_out(['ok' => false, 'msg' => "Estoque insuficiente ao finalizar: {$it['name']}."], 422);
        }
        ...
        $insSaida->execute([
          ...
          $it['unit'],
          (string)$it['qty'],
          (string)number_format((float)$it['price'], 2, '.', ''),
          (string)number_format((float)$it['line_total'], 2, '.', ''),
        ]);
      }
      ...
      json_out(['ok' => false, 'msg' => 'Erro ao salvar venda: ' . $e->getMessage()], 500);
    }

    // Retorno para o front (impressão + UI)
    json_out([
      'ok' => true,
      'msg' => "Venda {$pedido} confirmada!",
      'sale' => [
        'pedido' => $pedido,
        'date' => $dateBr,
        'customer' => $customer,
      ...
      'last' => last_sales($pdo, 10),
      'next_pedido' => next_pedido($pdo),
    ]);
  }

  json_out(['ok' => false, 'msg' => 'Ação inválida.'], 404);
}

// HTML (página)
$nextPedido = next_pedido($pdo);
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<body>
          ...
          <div class="row align-items-center">
            <div class="col-md-8">
              <div class="title">
                ...
                <?php if ($flash): ?>
                  <div class="mt-3 alert alert-<?= e((string)$flash['type']) ?> mb-0">
                    <?= e((string)$flash['msg']) ?>
                  </div>
                <?php endif; ?>
              </div>
            ...
            <div class="pdv-right-col">
              <div class="pdv-card">
                <div class="checkout-head">
                  ...
                  <span class="badge bg-light text-dark" id="saleNo"><?= e($nextPedido) ?></span>
                </div>
  ...
  <script>
    ...
    async function apiGet(action, params = {}) {
      ...
      const r = await fetch(`${API_URL}?${qs.toString()}`, { credentials: "same-origin" });
      const j = await r.json().catch(() => null);
      if (!j || !j.ok) throw new Error((j && j.msg) ? j.msg : "Erro na requisição.");
      return j;
    }
    ...
    async function apiPost(action, payload) {
      ...
      const j = await r.json().catch(() => null);
      if (!j || !j.ok) throw new Error((j && j.msg) ? j.msg : "Erro ao salvar.");
      return j;
    }
    ...
    function setPreview(prod) {
      ...
      previewSub.textContent = prod ? (`${prod.code} • Estoque: ${prod.stock} • ${numberToMoney(prod.price)}`) : "—";
    }
    ...
    function addToCart(prod) {
      ...
      if (idx >= 0) {
        ...
        if (nextQty > prod.stock) {
          alert("Estoque insuficiente para adicionar mais 1.");
          return;
        }
    ...
    function changeQty(id, delta) {
      ...
      if (!it) return;
      let v = Math.max(1, Number(it.qty || 1) + delta);
      if (v > it.stock) {
            ...
            <td>
              <div class="d-flex align-items-center gap-2">
                ...
                <div style="min-width:0;">
                  ...
                  <div class="muted">${safeText(it.code)} • Estoque: ${Number(it.stock)} • Unid: ${safeText(it.unit || "-")}</div>
                </div>
              </div>
            </td>
            <td>
    ...
    function renderLastSales(rows) {
      ...
      lastList.innerHTML = all.map(s => {
        const pedido = safeText(s.pedido || "—");
        const data = safeText(s.data || "");
        const total = Number(s.total || 0);
        const canal = safeText(s.canal || "");
        const pg = safeText(s.pagamento || "");
        return `
          <div class="cup">
            <div class="left">
              <div class="n">${pedido}</div>
              <div class="s">${data} • ${canal} • ${pg}</div>
            </div>
            <div class="right">
              <div class="v">${numberToMoney(total)}</div>
    ...
    async function refreshSuggest() {
      ...
      searchTimer = setTimeout(async () => {
        try {
          ...
          const r = await fetch(`${API_URL}?${qs.toString()}`, { signal: searchAbort.signal, credentials: "same-origin" });
          const j = await r.json().catch(() => null);
          if (!j || !j.ok) { showSuggest([]); return; }
    ...
    suggest.addEventListener("click", (e) => {
      ...
      const prod = SUGGEST_LIST.find(p => Number(p.id) === id);
      addToCart(prod);
      qProd.value = "";
      hideSuggest();
      qProd.focus();
    ...
    function buildReceiptHtml(sale) {
        ...
        return `
          <div class="item">
            ...
            <div class="row small">
              ...
              <span>${qty} x ${numberToMoney(price)} = ${numberToMoney(sub)}</span>
            </div>
          </div>
        `;
      }).join("");
      ...
      let payLines = "";
      if (sale.pay?.mode === "UNICO") {
        const m = (sale.pay?.selected || sale.pay?.method || "").toUpperCase();
        payLines = `<div class="row"><span>${safeText(m)}</span><span>${numberToMoney(Number(sale.total || 0))}</span></div>`;
      } else {
        const parts = (sale.pay?.parts || []).map(p => {
          const m = String(p.method || "").toUpperCase();
          const v = moneyToNumber(String(p.value ?? "0"));
          return `<div class="row"><span>${safeText(m)}</span><span>${numberToMoney(v)}</span></div>`;
        }).join("");
        payLines = parts || `<div class="row"><span>—</span><span>—</span></div>`;
      }
        ...
        <html>
          ...
          <body>
            <div class="wrap mono">
              ...
              <div class="center small">CUPOM (PDV)</div>
              ${hr()}
              <div class="row"><span class="bold">VENDA</span><span>${safeText(sale.pedido || "")}</span></div>
              ...
              ${hr()}
              <div class="row"><span>SUBTOTAL</span><span>${numberToMoney(Number(sale.totals?.sub || 0))}</span></div>
              <div class="row"><span>DESCONTO</span><span>- ${numberToMoney(Number(sale.totals?.desc || 0))}</span></div>
              <div class="row"><span>TAXA ENTREGA</span><span>${numberToMoney(Number(sale.totals?.fee || 0))}</span></div>
              ${hr()}
              <div class="row bold"><span>TOTAL</span><span>${numberToMoney(Number(sale.total || 0))}</span></div>
              ${hr()}
              <div class="row bold"><span>PAGAMENTO</span><span></span></div>
              ${payLines}
              <div class="row"><span>TROCO</span><span>${numberToMoney(Number(sale.pay?.troco || 0))}</span></div>
              ${hr()}
    ...
    async function refreshLastFromServer() {
      try {
        const j = await apiGet("last");
        renderLastSales(j.items || []);
        if (j.next_pedido) saleNo.textContent = j.next_pedido;
      } catch (e) {
        // silencioso
      }
    }
    ...
    async function confirmSale() {
      ...
      btnConfirmar.innerHTML = `<i class="lni lni-spinner-arrow me-1"></i> Salvando...`;
      ...
      try {
        ...
        const payload = {
          ...
          delivery: {
            mode: DELIVERY_MODE,
            address: String(cEndereco.value || "").trim(),
            fee: String(cEntrega.value || "0,00"),
            obs: String(cObs.value || "").trim(),
          },
          discount: {
            tipo: dTipo.value,
            valor: String(dValor.value || "0"),
          },
          pay: (() => {
            if (PAY_MODE === "UNICO") {
              const r = computeSinglePay();
              return {
                ...
                paid: String(pValor.value || "0,00"),
                troco: r.troco,
              };
            }
            const m = computeMultiPay();
            // pega os valores atuais das linhas
            const parts = Array.from(paysWrap.querySelectorAll(".pay-split-row")).map(row => ({
              method: row.querySelector(".mMethod")?.value || "PIX",
              value: row.querySelector(".mValue")?.value || "0,00",
            }));
            return { mode: "MULTI", parts };
          })(),
          items: CART.map(it => ({ produto_id: it.id, qty: it.qty })),
        };

        const j = await apiPost("confirm", payload);

        // imprime
        if (chkPrint.checked && j.sale) {
          const html = buildReceiptHtml(j.sale);
        ...
        // últimos cupons + próxima venda
        if (j.last) renderLastSales(j.last);
        if (j.next_pedido) saleNo.textContent = j.next_pedido;

        alert(j.msg || "Venda confirmada!");
        qProd.focus();
    ...
    function init() {
      ...
      btnRefreshLast.addEventListener("click", refreshLastFromServer);

      // carrega últimos do banco
      refreshLastFromServer();
    }
