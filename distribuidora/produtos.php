 ...
 * No banco está: images/arquivo.png
 * Você quer exibir no produtos.php como:
 *   ./assets/dados/produtos/images/arquivo.png
 */
function img_url_from_db(string $dbValue): string {
  $v = trim($dbValue);
  if ($v === '') return '';
  $v = ltrim($v, '/'); // remove barra inicial se existir
  return 'assets/dados/produtos/' . $v; // fica assets/dados/produtos/images/xxx.png
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<head>
  ...
  <style>
    ...
    .img-block { max-width: 320px; width: 100%; }
...
<body>
          ...
          <div class="row g-3 align-items-end">
            ...
            <div class="col-12 col-md-6 col-lg-3">
              <div class="d-grid gap-2 d-sm-flex justify-content-sm-end flex-wrap">
                <button class="main-btn primary-btn btn-hover btn-compact" data-bs-toggle="modal" data-bs-target="#modalProduto" id="btnNovo" type="button">
              ...
              <tbody>
                <?php foreach ($produtos as $p): ?>
                  <?php
                    ...
                    $imgDb = trim((string)($p['imagem'] ?? ''));      // ex: images/prod_xxx.png
                    $imgUrl = img_url_from_db($imgDb);                // vira assets/dados/produtos/images/prod_xxx.png
                  ?>
                  <tr
                    data-id="<?= $id ?>"
  ...
  <script>
    ...
    document.querySelectorAll("img.prod-img").forEach(img => {
      ...
      img.addEventListener('error', () => img.src = DEFAULT_IMG, { once: true });
    });
    ...
    function setPreview(src) { previewImg.src = src || DEFAULT_IMG; }
