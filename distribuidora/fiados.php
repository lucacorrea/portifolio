  ...
  <script>
    ...
    const FIADOS_DATA = [
      ...
      },
      {
        ...
        obs: "Cliente novo.",
        itens: [
          { codigo: "A5555", nome: "Água 20L", un: "UN", qtd: 2, preco: 15.00 },
        ],
        pagamentos: []
      },
      {
    ...
    function renderMeta() {
      ...
      document.getElementById("pillCount").textContent = `${count} fiados`;
      ...
      document.getElementById("lblRange").textContent = parts.length ? parts.join(" • ") : "—";
    }
    ...
    function renderTotals() {
      ...
      document.getElementById("tQtd").textContent = qtd;
      document.getElementById("tTot").textContent = brl(tot);
      document.getElementById("tPago").textContent = brl(pago);
      document.getElementById("tDeve").textContent = brl(deve);
      document.getElementById("tAberto").textContent = brl(aberto);
    }
    ...
    const mdReceber = new bootstrap.Modal(document.getElementById("mdReceber"));
    ...
    function openDetalhes(id) {
      ...
      document.getElementById("dtTitulo").textContent = `Detalhes do Fiado #${sale.id}`;
      document.getElementById("dtSub").textContent = `Data: ${fmtDateBR(sale.data)} • Canal: ${sale.canal}`;

      const pill = document.getElementById("dtStatusPill");
      pill.className = "pill " + (st === "QUITADO" ? "ok" : "warn");
      pill.textContent = st;

      document.getElementById("dtNome").textContent = sale.cliente.nome;
      document.getElementById("dtCpf").textContent = sale.cliente.cpf;
      document.getElementById("dtTel").textContent = sale.cliente.tel;
      document.getElementById("dtCanal").textContent = sale.canal;
      document.getElementById("dtObs").textContent = sale.obs || "—";

      document.getElementById("dtTotal").textContent = brl(total);
      document.getElementById("dtPago").textContent = brl(paid);
      document.getElementById("dtDeve").textContent = brl(deve);

      document.getElementById("dtVendaMeta").textContent =
        `${sale.itens.length} item(ns) • Total ${brl(total)} • Pago ${brl(paid)} • Deve ${brl(deve)}`;
    ...
    function openReceber(id) {
      ...
      document.getElementById("rcTitulo").textContent = `Receber • Fiado #${sale.id}`;
      document.getElementById("rcSub").textContent = `Data: ${fmtDateBR(sale.data)} • Canal: ${sale.canal}`;

      const pill = document.getElementById("rcStatusPill");
      pill.className = "pill " + (st === "QUITADO" ? "ok" : "warn");
      pill.textContent = st;

      document.getElementById("rcNome").textContent = sale.cliente.nome;
      document.getElementById("rcCpf").textContent = sale.cliente.cpf;
      document.getElementById("rcTotal").textContent = brl(total);
      document.getElementById("rcPago").textContent = brl(paid);
      document.getElementById("rcDeve").textContent = brl(deve);
      ...
      rcQuitar.checked = false;
      rcValor.value = "";
      rcHint.textContent = `Saldo atual: ${brl(deve)}. Digite um valor para pagar parcela, ou marque "Quitar tudo".`;
      ...
      document.getElementById("rcSalvar").onclick = () => {
        const sale2 = findSale(state.currentId);
        if (!sale2) return alert("Fiado não encontrado.");

        const total2 = sumSaleTotal(sale2);
        const paid2 = sumPaid(sale2);
        ...
        const v = Number(String(rcValor.value || "").replace(",", "."));
        if (!isFinite(v) || v <= 0) {
        ...
        // se estava com detalhes aberto, atualiza também (opcional)
        // (abrir detalhes automaticamente após receber)
        // openDetalhes(sale2.id);
      };
    ...
    document.getElementById("q").addEventListener("input", (e) => buildSuggest(e.target.value));
    ...
    document.getElementById("btnPdf").addEventListener("click", printView);

    // Enter no campo busca aplica filtro
    ...
    /* =========================
       INIT
    ========================== */
    // Normaliza "QUITADOS" no dataset (pra ficar coerente)
    // (se deve = 0, já considera quitado)
    function init() {
      state.per = Number(document.getElementById("per").value) || 25;
