...
function table_missing(Throwable $e): bool
{
  ...
  return str_contains($m, 'doesn\'t exist') || str_contains($m, 'not found') || str_contains($m, 'unknown table');
}

/* =========================================================
   PRODUTOS 1x (SEM AJAX) p/ autocomplete do produto na devolução parcial
========================================================= */
$PRODUTOS_CACHE = [];
try {
  $stP = $pdo->query("
...
if (isset($_GET['ajax'])) {
  ...
  try {
    ...
    if ($ajax === 'save') {
      ...
      $saleNo = trim((string)($payload['saleNo'] ?? ''));
    ...
    if (table_missing($e)) {
      json_out(['ok' => false, 'msg' => "Tabela 'devolucoes' não encontrada. Rode o SQL de criação da tabela devolucoes."], 500);
    }
...
<body>
  ...
  <script>
    ...
    prodSuggest.addEventListener("click", (e) => {
      ...
      const p = LAST_PROD.find(x => Number(x.id) === id);
      if (!p) return;
    ...
    document.addEventListener("click", (e) => {
      if (!e.target.closest(".search-wrap")) hideProdSuggest();
    });
      ...
      rows.forEach((x) => {
        ...
        tbodyDev.insertAdjacentHTML("beforeend", `
          <tr data-id="${Number(x.id)}">
            ...
            <td>
              ${safeText(motivo)}
              ${x.note ? `<div class="muted" style="max-width:260px;white-space:normal;">${safeText(x.note)}</div>` : ""}
            </td>
    ...
    async function saveDev() {
      ...
      try {
        ...
        const item = r.item;
        const idx = DEV.findIndex(x => Number(x.id) === Number(item.id));
        const norm = {
          id: Number(item.id),
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Escape") hideProdSuggest();
    });
