...
$frontendContext = [
    ...
    'comidaMesa' => [
        'competenciaId' => $filter->competenceId,
        'permissions' => [
...
<body data-page="modulo">
                ...
                <section class="module-hero" aria-labelledby="moduleTitle">
                    <div class="module-hero-content">
                        ...
                        <div class="module-hero-actions page-actions">
                            <button class="btn btn-light" type="button"<?= $canManageCompetences ? ' data-open-competence' : ' disabled title="Sem permissão para gerenciar competências"' ?>><i class="bi bi-calendar-plus"></i><span class="optional">Gerenciar competência</span></button>
                    ...
                    <div data-tab-panel="beneficiaries">
                        ...
                        <form class="filter-bar" id="filterForm" method="get" action="modulo.php" data-server-filter aria-label="Filtros da listagem">
                            <div class="filter-row">
                                ...
                                <div><label class="form-label" for="competenceFilter">Competência</label><div class="input-group"><select class="form-select" id="competenceFilter" name="competencia_id"><option value="">Padrão</option><?php foreach ($competences as $item): ?><option value="<?= e($item['id']) ?>"<?= selected($filter->competenceId, $item['id']) ?>><?= e($service->formatCompetence((int) $item['mes'], (int) $item['ano'])) ?></option><?php endforeach; ?></select><button class="btn btn-light" type="button"<?= $canManageCompetences ? ' data-open-competence' : ' disabled title="Sem permissão"' ?> aria-label="Gerenciar competência"><i class="bi bi-gear"></i></button></div></div>
                        ...
                        <section class="content-card table-card" aria-labelledby="beneficiaryListTitle">
                            <div class="table-toolbar">
                                ...
                                <div class="d-flex gap-2"><button class="btn btn-light btn-sm" type="button"<?= $canManageCompetences ? ' data-open-competence' : ' disabled title="Sem permissão"' ?>><i class="bi bi-calendar-plus"></i>Competência</button></div>
                            </div>
                                ...
                                <table class="data-table" id="beneficiaryTable">
                                    ...
                                    <tbody>
                                        <?php foreach ($items as $row): ?>
                                            <?php [$programClass, $programIcon] = program_status_meta((string) $row['inscricao_status']); $delivery = $service->deliveryStatusForRow($row, $competence); $competenceOpen = $competence !== null && ($competence['status'] ?? null) === 'aberta'; $canDeliverRow = $canDeliver && (string) $row['inscricao_status'] === 'ativa' && $competenceOpen && ($row['entrega_status'] ?? null) !== 'entregue'; $canCancelRow = $canCancelDelivery && ($row['entrega_status'] ?? null) === 'entregue'; $deliveryTitle = !$competenceOpen ? 'Competência não está aberta' : ((string) $row['inscricao_status'] !== 'ativa' ? 'Inscrição não está ativa' : (($row['entrega_status'] ?? null) === 'entregue' ? 'Cesta já entregue nesta competência' : 'Registrar entrega')); ?>
                                            <tr>
                                                ...
                                                <td><div class="table-actions justify-content-end"><button class="btn btn-light btn-sm" type="button" data-open-detail data-registration-id="<?= e($row['inscricao_id']) ?>"><i class="bi bi-eye"></i>Visualizar</button><div class="dropdown"><button class="btn btn-light btn-icon btn-sm" type="button" data-bs-toggle="dropdown" aria-label="Ações de <?= e($row['responsavel_nome']) ?>"><i class="bi bi-three-dots"></i></button><ul class="dropdown-menu dropdown-menu-end"><?php if ($canConsultCpf): ?><li><button class="dropdown-item" type="button" data-bs-toggle="modal" data-bs-target="#newRegistrationModal" data-consult-cpf="<?= e($row['cpf']) ?>"><i class="bi bi-search me-2"></i>Consultar CPF</button></li><?php endif; ?><li><button class="dropdown-item" type="button"<?= $canEdit ? ' data-open-edit data-registration-id="' . e($row['inscricao_id']) . '"' : ' disabled title="Sem permissão"' ?>><i class="bi bi-pencil me-2"></i>Editar cadastro</button></li><li><button class="dropdown-item" type="button"<?= $canDeliverRow ? ' data-open-delivery data-registration-id="' . e($row['inscricao_id']) . '" data-registration-name="' . e($row['responsavel_nome']) . '" data-family-code="' . e($row['familia_codigo']) . '"' : ' disabled title="' . e($deliveryTitle) . '"' ?>><i class="bi bi-basket2 me-2"></i><?= ($row['entrega_status'] ?? null) === 'cancelada' ? 'Reativar entrega' : 'Registrar entrega' ?></button></li><li><button class="dropdown-item" type="button"<?= $canCancelRow ? ' data-open-cancel data-registration-id="' . e($row['inscricao_id']) . '" data-family-code="' . e($row['familia_codigo']) . '"' : ' disabled title="Cancelamento indisponível"' ?>><i class="bi bi-x-circle me-2"></i>Cancelar entrega</button></li><li><button class="dropdown-item" type="button"<?= $canSendDocuments ? ' data-open-document data-registration-id="' . e($row['inscricao_id']) . '"' : ' disabled title="Sem permissão"' ?>><i class="bi bi-paperclip me-2"></i>Enviar documento</button></li></ul></div></div></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
                        ...
                        <section class="mobile-records" aria-label="Famílias beneficiárias em cartões">
                            <?php foreach ($items as $row): ?>
                                <?php [$programClass] = program_status_meta((string) $row['inscricao_status']); $delivery = $service->deliveryStatusForRow($row, $competence); $competenceOpen = $competence !== null && ($competence['status'] ?? null) === 'aberta'; $canDeliverRow = $canDeliver && (string) $row['inscricao_status'] === 'ativa' && $competenceOpen && ($row['entrega_status'] ?? null) !== 'entregue'; ?>
                                <article class="mobile-record-card">
                                    ...
                                    <div class="mobile-record-actions"><button class="btn btn-primary flex-grow-1" type="button"<?= $canDeliverRow ? ' data-open-delivery data-registration-id="' . e($row['inscricao_id']) . '" data-registration-name="' . e($row['responsavel_nome']) . '" data-family-code="' . e($row['familia_codigo']) . '"' : ' disabled title="Entrega indisponível"' ?>><i class="bi bi-basket2"></i><?= ($row['entrega_status'] ?? null) === 'cancelada' ? 'Reativar entrega' : 'Registrar entrega' ?></button><button class="btn btn-light btn-icon" type="button"<?= $canEdit ? ' data-open-edit data-registration-id="' . e($row['inscricao_id']) . '"' : ' disabled title="Sem permissão"' ?>><i class="bi bi-pencil"></i></button></div>
                                </article>
                            <?php endforeach; ?>
                        </section>
                    </div>
    ...
    <div class="modal fade" id="registrationFormModal" tabindex="-1" aria-labelledby="registrationFormTitle" aria-hidden="true">
        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable"><div class="modal-content"><form id="registrationForm" action="api/comida-mesa/salvar-cadastro.php" method="post" novalidate>
            ...
            <div class="modal-body">
                <input type="hidden" name="_csrf" value="<?= e($frontendContext['csrf']['salvarCadastro']) ?>"><input type="hidden" name="inscricao_id">
    ...
    <div class="modal fade" id="deliveryModal" tabindex="-1" aria-labelledby="deliveryTitle" aria-hidden="true"><div class="modal-dialog modal-dialog-centered modal-dialog-scrollable"><div class="modal-content"><form id="deliveryForm" action="api/comida-mesa/registrar-entrega.php" method="post" novalidate><div class="modal-header"><div><div class="eyebrow mb-1"><i class="bi bi-basket2"></i>Entrega</div><h2 class="modal-title fs-5" id="deliveryTitle">Registrar entrega</h2></div><button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Fechar"></button></div><div class="modal-body"><input type="hidden" name="_csrf" value="<?= e($frontendContext['csrf']['registrarEntrega']) ?>"><input type="hidden" name="inscricao_id"><input type="hidden" name="competencia_id" value="<?= e($filter->competenceId) ?>"><div data-form-alert></div><div class="alert-soft info mb-3"><i class="bi bi-info-circle"></i><div><strong data-delivery-family>Família</strong><br><span><?= e($competenceLabel) ?></span></div></div><div class="mb-3"><label class="form-label">Nome do recebedor</label><input class="form-control" name="recebedor_nome" required></div><div class="mb-3"><label class="form-label">CPF do recebedor</label><input class="form-control" name="recebedor_cpf" inputmode="numeric"></div><div class="mb-3"><label class="form-label">Parentesco</label><input class="form-control" name="recebedor_parentesco"></div><div class="mb-3"><label class="form-label">Observação</label><textarea class="form-control" name="observacao" rows="2"></textarea></div></div><div class="modal-footer"><button class="btn btn-light" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn btn-primary" type="submit"><i class="bi bi-check2"></i>Registrar</button></div></form></div></div></div>

    <div class="modal fade" id="cancelDeliveryModal" tabindex="-1" aria-labelledby="cancelDeliveryTitle" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><form id="cancelDeliveryForm" action="api/comida-mesa/cancelar-entrega.php" method="post" novalidate><div class="modal-header"><div><div class="eyebrow mb-1"><i class="bi bi-x-circle"></i>Cancelamento</div><h2 class="modal-title fs-5" id="cancelDeliveryTitle">Cancelar entrega</h2></div><button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Fechar"></button></div><div class="modal-body"><input type="hidden" name="_csrf" value="<?= e($frontendContext['csrf']['cancelarEntrega']) ?>"><input type="hidden" name="inscricao_id"><input type="hidden" name="competencia_id" value="<?= e($filter->competenceId) ?>"><div data-form-alert></div><p class="text-secondary" data-cancel-family></p><label class="form-label">Motivo</label><textarea class="form-control" name="motivo" rows="4" minlength="10" maxlength="255" required></textarea></div><div class="modal-footer"><button class="btn btn-light" type="button" data-bs-dismiss="modal">Voltar</button><button class="btn btn-danger" type="submit"><i class="bi bi-x-lg"></i>Cancelar entrega</button></div></form></div></div></div>

    <div class="modal fade" id="competenceModal" tabindex="-1" aria-labelledby="competenceTitle" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><form id="competenceForm" action="api/comida-mesa/salvar-competencia.php" method="post" novalidate><div class="modal-header"><div><div class="eyebrow mb-1"><i class="bi bi-calendar-event"></i>Competência</div><h2 class="modal-title fs-5" id="competenceTitle">Gerenciar competência</h2></div><button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Fechar"></button></div><div class="modal-body"><input type="hidden" name="_csrf" value="<?= e($frontendContext['csrf']['salvarCompetencia']) ?>"><input type="hidden" name="competencia_id" value="<?= e($filter->competenceId) ?>"><div data-form-alert></div><div class="row g-3"><div class="col-6"><label class="form-label">Mês</label><input class="form-control" name="mes" type="number" min="1" max="12" value="<?= e($competence['mes'] ?? date('n')) ?>"></div><div class="col-6"><label class="form-label">Ano</label><input class="form-control" name="ano" type="number" min="2020" value="<?= e($competence['ano'] ?? date('Y')) ?>"></div><div class="col-12"><label class="form-label">Situação</label><select class="form-select" name="status"><option value="planejada">Planejada</option><option value="aberta"<?= selected($competence['status'] ?? '', 'aberta') ?>>Aberta</option><option value="encerrada">Encerrada</option><option value="cancelada">Cancelada</option></select></div><div class="col-6"><label class="form-label">Início</label><input class="form-control" name="inicio_entregas" type="date" value="<?= e($competence['inicio_entregas'] ?? '') ?>"></div><div class="col-6"><label class="form-label">Fim</label><input class="form-control" name="fim_entregas" type="date" value="<?= e($competence['fim_entregas'] ?? '') ?>"></div><div class="col-12"><label class="form-label">Observação</label><textarea class="form-control" name="observacao" rows="2"><?= e($competence['observacao'] ?? '') ?></textarea></div></div></div><div class="modal-footer"><button class="btn btn-light" type="button" data-bs-dismiss="modal">Cancelar</button><button class="btn btn-primary" type="submit"><i class="bi bi-save"></i>Salvar</button></div></form></div></div></div>
