...
try {
    ...
    if (!Csrf::validate($_POST['_csrf'] ?? null, 'comida_mesa_registrar_entrega')) {
    ...
    if (!$authorization->can($user, 'comida_mesa.entregar')) {
        cm_json(403, ['ok' => false, 'error' => 'Acesso negado.']);
    }

    $result = (new ComidaMesaService(new ComidaMesaRepository($pdo)))->registerDelivery(
        ComidaMesaEntregaData::fromArray($_POST),
        $user->id,
        $audit
    );

    cm_json($result['reactivated'] ? 200 : 201, [
        'ok' => true,
        'message' => $result['reactivated'] ? 'Entrega reativada.' : 'Entrega registrada.',
        'data' => $result,
    ]);
} catch (RuntimeException $exception) {
    cm_error($exception);
} catch (Throwable $exception) {
    Logger::application('Comida Mesa delivery failed.', [
        'type' => $exception::class,
        'code' => $exception->getCode(),
    ]);
    cm_json(500, ['ok' => false, 'error' => 'Não foi possível registrar a entrega.']);
}
