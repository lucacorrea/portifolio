...
try {
    ...
    if (!$authorization->can($user, 'comida_mesa.visualizar')) {
        cm_json(403, ['ok' => false, 'error' => 'Acesso negado.']);
    }

    $id = isset($_GET['id']) && preg_match('/^\d+$/', (string) $_GET['id']) === 1
        ? (int) $_GET['id']
        : 0;

    if ($id < 1) {
        cm_json(400, ['ok' => false, 'error' => 'Inscrição não informada.']);
    }

    $service = new ComidaMesaService(new ComidaMesaRepository($pdo));
    $data = $service->detail(
        $id,
        $authorization->can($user, 'comida_mesa.editar'),
        $authorization->can($user, 'comida_mesa.documentos_visualizar'),
        $authorization->can($user, 'comida_mesa.historico_visualizar')
    );
    ...
    cm_json($status, [
        'ok' => false,
        'error' => $exception->getCode() === 404 ? 'Registro inexistente.' : $exception->getMessage(),
    ]);
} catch (Throwable $exception) {
    Logger::application('Comida Mesa detail failed.', [
