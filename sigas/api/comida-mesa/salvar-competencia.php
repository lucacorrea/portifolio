...
try {
    ...
    if (!Csrf::validate($_POST['_csrf'] ?? null, 'comida_mesa_salvar_competencia')) {
    ...
    if (!$authorization->can($user, 'comida_mesa.competencias_gerenciar')) {
        cm_json(403, ['ok' => false, 'error' => 'Acesso negado.']);
    }

    $data = ComidaMesaCompetenciaData::fromArray($_POST);
    $id = (new ComidaMesaService(new ComidaMesaRepository($pdo)))->saveCompetence($data, $user->id, $audit);

    cm_json($data->id === null ? 201 : 200, [
        'ok' => true,
        'message' => 'Competência salva.',
        'data' => ['id' => $id],
    ]);
} catch (RuntimeException $exception) {
    cm_error($exception);
} catch (Throwable $exception) {
    Logger::application('Comida Mesa competence failed.', [
        'type' => $exception::class,
        'code' => $exception->getCode(),
    ]);
    cm_json(500, ['ok' => false, 'error' => 'Não foi possível salvar a competência.']);
}
