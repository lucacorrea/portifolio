...
function cm_int(string $key): int
{
    return isset($_POST[$key]) && preg_match('/^\d+$/', (string) $_POST[$key]) === 1
        ? (int) $_POST[$key]
        : 0;
}
...
try {
    ...
    if (!Csrf::validate($_POST['_csrf'] ?? null, 'comida_mesa_cancelar_entrega')) {
    ...
    if (!$authorization->can($user, 'comida_mesa.cancelar_entrega')) {
        cm_json(403, ['ok' => false, 'error' => 'Acesso negado.']);
    }

    $id = (new ComidaMesaService(new ComidaMesaRepository($pdo)))->cancelDelivery(
        cm_int('inscricao_id'),
        cm_int('competencia_id'),
        (string) ($_POST['motivo'] ?? ''),
        $user->id,
        $audit
    );

    cm_json(200, ['ok' => true, 'message' => 'Entrega cancelada.', 'data' => ['id' => $id]]);
} catch (RuntimeException $exception) {
    cm_error($exception);
} catch (Throwable $exception) {
    Logger::application('Comida Mesa cancel delivery failed.', [
        'type' => $exception::class,
        'code' => $exception->getCode(),
    ]);
    cm_json(500, ['ok' => false, 'error' => 'Não foi possível cancelar a entrega.']);
}
