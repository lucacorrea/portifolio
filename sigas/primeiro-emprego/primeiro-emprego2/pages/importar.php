...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pe_action']) && in_array($_POST['pe_action'], ['validate_import', 'run_import'], true)) {
    try {
        pe_verify_csrf();
        if (!$dbReady) {
            throw new RuntimeException('Execute a atualização 003 do banco antes da importação.');
        }
...
<section class="content-card pe-form-card">
    <?php if (!$dbReady): ?>
        <div class="alert alert-warning mb-3">
            ...
            Como o banco já está na hospedagem, execute apenas <code>database/003_revisao_sem_bloqueios.sql</code> antes de usar esta versão.
        </div>
    <?php endif; ?>
