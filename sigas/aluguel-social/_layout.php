...
if (!isset($frontendContext['navigation'][$environmentKey])) {
    ...
    $errorMessage = 'Seu setor, nível ou exceção individual não permite acessar o módulo Aluguel Social.';
    require dirname(__DIR__) . '/frontend/layouts/error-layout.php';
    return;
}
...
if (!sigas_operational_can_page($environmentKey, $pageKey)) {
    ...
    $errorMessage = 'Você possui acesso ao Aluguel Social, mas não tem permissão para esta área específica.';
    require dirname(__DIR__) . '/frontend/layouts/error-layout.php';
    return;
}
