...
$identityMigration = skf_read($root . '/database/migrations/20260907_018_pessoas_identificacao_socioeconomica.sql');
...
$screenJs = skf_read($root . '/assets/js/prontuario-socioeconomico.js');
...
foreach (['pessoa_socioeconomico','pessoa_socioeconomico_historico','beneficio_solicitacoes','kit_maternidade_solicitacoes','kit_maternidade_acompanhamentos','kit_maternidade_avaliacoes','kit_maternidade_entregas'] as $table) {
...
foreach (['genero','cor_raca','estado_civil','naturalidade','nacionalidade','rg_emissao','rg_uf','whatsapp'] as $column) {
    skf_assert(str_contains($identityMigration, 'ADD COLUMN IF NOT EXISTS ' . $column), 'migration 018 deve criar pessoas.' . $column);
    skf_assert(str_contains($personRepository, "'" . $column . "' =>"), 'repositório central deve persistir ' . $column);
}
...
skf_assert(str_contains($socialService, "'history' => \$history"), 'consulta deve expor histórico resumido sem snapshots brutos');
...
foreach (['Prontuário Socioeconômico','Visão geral','Composição familiar','Condições habitacionais','Vulnerabilidades','Histórico do prontuário','O prontuário não aprova benefícios'] as $text) {
...
skf_assert(str_contains($screen, 'data-socio-tab="overview"'), 'prontuário deve usar navegação por abas');
skf_assert(str_contains($screen, 'Editar / atualizar'), 'prontuário deve separar consulta de edição');
skf_assert(str_contains($screenJs, 'setEditMode'), 'javascript deve controlar modo consulta/edição');
skf_assert(str_contains($screenJs, 'renderHistory'), 'javascript deve renderizar histórico resumido');
...
foreach ([$root . '/scripts/preflight-socioeconomic-kit.php',$root . '/scripts/verify-socioeconomic-kit.php',$root . '/assets/css/prontuario-socioeconomico.css',$root . '/assets/js/prontuario-socioeconomico.js',$root . '/assets/js/modules/kit-maternidade-flow.js'] as $requiredFile) {
